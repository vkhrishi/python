...
def walk_forward(trades, candles, folds=4):
    ...
    if passed:
        print("   -> Edge holds out-of-sample. Confirm on REAL premiums")
        print("      (bot.py backtest_real) then forward paper-trade.")
    else:
...
def main():
    ...
    if not path:
        ...
        print("Usage: python backtest.py <data.csv|data.json> [folds] [--htf] [--adx N] [--rr N]\n")
