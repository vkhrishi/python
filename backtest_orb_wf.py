...
def collect_trades(groww, candles):
    ...
    days_total = days_with_orb = vol_skipped = 0
    reason_tally = {}
    ...
    for day in days_seen:
        ...
        for c in candles:
            ...
            sig = res.get("signal")
            if res.get("details", {}).get("vol_ratio") == "N/A (no vol data)":
                vol_skipped += 1
...
def _print_breakdown(trades, diag):
    ...
    vs = diag.get("vol_skipped", 0)
    if vs:
        print("   Volume filter        : SKIPPED on %d candle evaluations (index has no vol data)" % vs)
