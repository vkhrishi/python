...
#
#  USAGE (on the VPS):
#    python backtest_real_wf.py [days] [folds] [--htf] [--adx N] [--rr N]
...
def _fetch_history_capture(groww, days=30):
    candles = _orig_fetch_history(groww, days=days)
...
def main():
    ...
    # Skip the value that follows --adx / --rr so it isn't read as days/folds.
    ints = []
    skip = False
    for a in args:
        if skip:
    ...
    print("=" * 64)
    print(" REAL-PREMIUM VALIDATION | days=%d folds=%d" % (days, folds))
    if overrides:
