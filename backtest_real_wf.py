...
# -------------------------------------------------------------------
#  Option-history fix.
#  bot.py builds an OLD live-trading symbol (NIFTY26...) and calls the
#  DEPRECATED get_historical_candle_data - which never resolves expired
#  FNO contracts, so every signal day is skipped. The working path is
#  get_historical_candles(groww_symbol="NSE-NIFTY-DDMmmYY-STRIKE-TYPE").
#  We override both, WITHOUT touching bot.py.
# -------------------------------------------------------------------
...
def _build_symbol_groww(strike, opt_type, expiry=None):
    """Emit a Groww backtesting symbol: NSE-NIFTY-30Sep25-24650-CE."""
    if expiry is None:
        expiry = bot.get_expiry_date()
    return "NSE-NIFTY-%s-%d-%s" % (expiry.strftime("%d%b%y"), int(round(strike)), opt_type)


def _real_expiry_on_or_after(groww, day):
    """Nearest LISTED NIFTY expiry on/after the signal day (handles holiday
    shifts that a fixed weekly-Tuesday guess would miss). Cached per month."""
    ...
    for e in sorted(set(cands)):
        if e >= d0:
            return e
    return None


def _fetch_option_history_groww(groww, symbol, day):
    """Fetch 5-min option premiums via the CORRECT backtesting API.
    `symbol` here is the groww-symbol produced by _build_symbol_groww."""
    parts = symbol.split("-")
    opt_type, strike = parts[-1], parts[-2]
    exp = _real_expiry_on_or_after(groww, day)
    gsym = symbol
    if exp is not None:
        gsym = "NSE-NIFTY-%s-%s-%s" % (exp.strftime("%d%b%y"), strike, opt_type)
...
def _install_capture():
    ...
    bot.build_symbol = _build_symbol_groww
    bot._fetch_option_history = _fetch_option_history_groww
