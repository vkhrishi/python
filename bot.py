...
LOT_SIZE         = 65
LOTS_TO_TRADE    = 1
OTM_OFFSET       = 0       # ATM options (better liquidity + decent delta)

# -- PAPER TRADE MODE --
PAPER_TRADE      = True     # START WITH PAPER - validate first!
...
MIN_IMPULSE_PCT      = 0.15   # 0.15% of Nifty (e.g., ~37 pts at 25000)
MAX_IMPULSE_PCT      = 1.00   # Max 1% (avoid exhaustion moves)
MIN_IMPULSE_POINTS   = 30     # Absolute minimum
MAX_IMPULSE_POINTS   = 250    # Absolute maximum

# -- Bounce confirmation --
BOUNCE_MIN_BODY_PCT  = 0.45   # Candle body >= 45% of range
...
MAX_DAILY_LOSS_RUPEES = 4000   # Allow 2 losses max
...
MIN_ADX_FOR_TRADE    = 18      # Slightly higher - need trending
ADX_LEN              = 14
ATR_LEN              = 14
RSI_LEN              = 14
EMA_FAST             = 9
...
TRAILING_SL_STEP     = 0.50    # Trail 50% of profit
...
def check_fib_retracement(candles, fib_data):
    ...
    is_late = now.hour >= 11
    if is_late:
        min_score = 5  # Higher bar for late entries
...
def main():
    ...
    strike = atm + OTM_OFFSET if opt_type == "PE" else atm - OTM_OFFSET
