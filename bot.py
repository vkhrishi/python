...
def place_oco_order(groww, symbol, qty, entry_premium, sl_price, target_price):
    if PAPER_TRADE:
        fake_id = f"PAPER-OCO-{ist_now().strftime('%H%M%S')}"
        logging.info(f"?? PAPER OCO | E:Rs.{entry_premium} SL:Rs.{sl_price} T:Rs.{target_price}")
        return fake_id
    ...
    # Format prices as strings (Groww requires decimal strings)
    target_trigger = fmt(target_price)       # Trigger AT target price
    target_limit   = fmt(target_price)       # Limit = same as target
    sl_trigger     = fmt(sl_price)           # Trigger AT stop loss price

    logging.info(f"OCO PARAMS | target_trigger={target_trigger} target_limit={target_limit} sl_trigger={sl_trigger}")
    ...
    try:
        ...
        logging.warning(f"?? MANUAL: SL=Rs.{sl_price} T=Rs.{target_price}")
        return None
...
def main():
    ...
    # Accept both HIGH and MED confidence
