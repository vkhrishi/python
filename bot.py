...
def main():
    ...
    # Check how many trades already taken this session (allow up to MAX_TRADES_SESSION)
    session_trades = sum(1 for t in state.get("trades", []) if t.get("session") == session_name)
    if session_trades >= MAX_TRADES_SESSION:
        logging.info("[%s] %s session trade limit reached (%d/%d)" % (
            now.strftime("%H:%M"), session_name, session_trades, MAX_TRADES_SESSION))
        return
