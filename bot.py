...
#  FIXED VERSION v3 — Monday expiry + dynamic expiry discovery
...
#  NSE shifted NIFTY weekly expiry from Thursday → Monday
#  in 2025. Change this if NSE changes it again.
# ═══════════════════════════════════════════════════════════
NIFTY_EXPIRY_WEEKDAY = 0   # 0=Monday, 1=Tuesday, 3=Thursday
...
# ═════════════════════════════════════════════════════════════
#  SECTION 5 — UTILITIES (FIXED EXPIRY LOGIC)
# ═════════════════════════════════════════════════════════════
...
def get_expiry_date():
    """Get next NIFTY weekly expiry.

    NIFTY weekly expiry = Monday (NIFTY_EXPIRY_WEEKDAY=0).
    - If today IS expiry day and market is still open → use today.
    - If today IS expiry day but market closed → use next week.
    - Otherwise → use the coming expiry day.
    """
    today = ist_now().date()
    now = ist_now()
    target_weekday = NIFTY_EXPIRY_WEEKDAY  # 0=Monday
    ...
    logging.info(f"Expiry calc: today={today}({today.strftime('%A')}) target_weekday={target_weekday} → expiry={expiry}({expiry.strftime('%A')})")
    return expiry
...
def build_symbol(strike, opt_type, expiry=None):
    """Build NSE option symbol: NIFTY2641323650PE
...
def login(force=False):
    ACCESS_TOKEN = "eyJraWQiOiJaTUtjVXciLCJhbGciOiJFUzI1NiJ9..uCPPlMPHckdt9PVZtwf7ZHsvx4YtwrOhtVKfXkMIeW2hB3RwNnFUrRk92uigLQS5UOwuXHH72hdsk4PfyLlAPg"
    groww = GrowwAPI(ACCESS_TOKEN)
    logging.info("Groww login OK (direct access token)")
    return groww
...
def _discover_expiry_from_csv(groww, strike, opt_type):
    """Find the nearest valid expiry date from instrument CSV.
    Returns (expiry_date_str, trading_symbol) or (None, None).
    """
    try:
...
def _discover_expiry_from_chain(groww, strike, opt_type, calculated_expiry):
    """Try nearby expiry dates via option chain API when calculated expiry returns 0 strikes.
    Tries: today, today+1 ... today+7 (skipping the already-failed date).
    """
    today = ist_now().date()
    calc_str = calculated_expiry.strftime("%Y-%m-%d")
    
    # Try dates: today through today+7
    candidates = []
    ...
    for exp_str in candidates:
        try:
            ...
            logging.info(f"Chain discovery: expiry={exp_str} has {len(strikes_data)} strikes")

            # Check if our strike exists
            strike_str = str(strike)
            for sk in [strike_str, f"{strike}.0"]:
                if sk in strikes_data and opt_type in strikes_data[sk]:
                    ...
                    if tsym:
                        logging.info(f"FOUND via chain discovery: {tsym} expiry={exp_str} LTP={ltp}")
                        return exp_str, tsym, ltp, strikes_data

            # Strike not found but chain has data — find nearest strike
            available = []
            for k, v in strikes_data.items():
                if opt_type in v:
                    try:
                        available.append(int(float(k)))
                    except:
                        continue
            if available:
                ...
                if nearest_key:
                    ...
                    ltp = float(opt_data.get("ltp", 0))
                    if tsym:
                        logging.info(f"FOUND nearest via chain discovery: {tsym} strike={nearest} expiry={exp_str} LTP={ltp}")
                        return exp_str, tsym, ltp, strikes_data

        except Exception as e:
            logging.debug(f"Chain discovery expiry={exp_str} failed: {e}")
            continue
...
def get_valid_option_symbol(groww, strike, opt_type):
    """Resolve option symbol with 4-tier fallback:
    1. Direct symbol build + quote
    2. Option chain API with calculated expiry
    3. Dynamic expiry discovery via chain (try today thru today+7)
    4. Instrument CSV lookup with nearest valid expiry
    """
    expiry = get_expiry_date()
    ...
    # ─── TRY 2: Option chain with calculated expiry ───
    chain_found = False
    try:
        chain = groww.get_option_chain(
            exchange="NSE",
        ...
        if isinstance(chain, dict):
            ...
            logging.info(f"Option chain loaded: {num_strikes} strikes, underlying_ltp={chain.get('underlying_ltp', 'N/A')}")
            ...
            if num_strikes > 0:
                        ...
                        if tsym:
                            if ltp <= 0:
                                try:
                                    ...
                                    pass
                            logging.info(f"Symbol OK (chain): {tsym} | LTP: Rs.{ltp}")
                            return tsym, ltp
                ...
                logging.warning(f"Option chain returned 0 strikes for expiry {expiry_str} — wrong expiry day!")
    except Exception as e:
        logging.error(f"Option chain API call failed: {e}")

    # ─── TRY 3: Dynamic expiry discovery via chain ───
    if not chain_found:
        logging.info(f"Trying dynamic expiry discovery (today thru today+7)...")
        disc_expiry, disc_sym, disc_ltp, disc_strikes = _discover_expiry_from_chain(
            groww, strike, opt_type, expiry
        )
        if disc_sym:
            if disc_ltp <= 0:
                try:
                    ...
                    pass
            logging.info(f"Symbol OK (chain discovery): {disc_sym} | Expiry: {disc_expiry} | LTP: Rs.{disc_ltp}")
            return disc_sym, disc_ltp

    # ─── TRY 4: Instrument CSV — find nearest valid expiry ───
    ...
    if csv_sym:
        logging.info(f"CSV found: {csv_sym} expiry={csv_expiry}")
        try:
            q = groww.get_quote(exchange="NSE", segment="FNO", trading_symbol=csv_sym)
            ltp = float(q.get("last_price", 0))
            if ltp > 0:
                logging.info(f"Symbol OK (CSV): {csv_sym} | LTP: Rs.{ltp}")
                return csv_sym, ltp
        except Exception as e:
            logging.warning(f"CSV symbol {csv_sym} quote failed: {e}")
        # Return even with 0 LTP
        return csv_sym, 0

    # Also try nearby strikes in CSV
    for offset in [50, 100]:
        for adj_strike in [strike + offset, strike - offset]:
            csv_expiry2, csv_sym2 = _discover_expiry_from_csv(groww, adj_strike, opt_type)
            if csv_sym2:
                logging.info(f"CSV found (nearby strike {adj_strike}): {csv_sym2} expiry={csv_expiry2}")
                try:
                    q = groww.get_quote(exchange="NSE", segment="FNO", trading_symbol=csv_sym2)
                    ltp = float(q.get("last_price", 0))
                    if ltp > 0:
                        logging.info(f"Symbol OK (CSV nearby): {csv_sym2} | LTP: Rs.{ltp}")
                        return csv_sym2, ltp
                except:
                    pass
                return csv_sym2, 0
