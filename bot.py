...
def main():
    ...
    logging.info(f"=== LIVE | {now.strftime('%Y-%m-%d %H:%M:%S')} | Trades:{state['trade_count']}/{MAX_TRADES_DAY} | P&L:Rs.{state.get('daily_pnl_rupees',0):+.0f} ===")
    ...
    state["trades"].append({
        ...
        "rr": f"1:{result.get('target_premium',0)/max(result.get('risk_premium',1),0.01):.1f}",
    ...
    logging.info(f"LIVE TRADE | {signal} {confidence} | {symbol}")
