...
RR_RATIO          = 2.0     # Target = SL x 2.5 (1:2.5 RR)
...
MIN_HOLD_MINUTES      = 5       # No normal SL exit before 5 minutes
        ...
        while True:
            ...
            if now_ts - last_log >= 30:
                logging.info("  POLL | LTP:%.1f | P&L:%+.0f | SL:%.1f T:%.1f | Hold:%.0fm" % (
                    current_ltp, pnl, sl_price, target_price, minutes_held))
                last_log = now_ts
