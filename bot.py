...
IMPULSE_START_HOUR    = 9       # Impulse leg starts at market open
IMPULSE_START_MIN     = 15
IMPULSE_END_HOUR      = 9      # Impulse leg ends (wait for swing to form)
IMPULSE_END_MIN       = 45
MIN_IMPULSE_POINTS    = 30     # Minimum swing size to be valid impulse
MAX_IMPULSE_POINTS    = 300    # Reject if impulse too large (exhaustion)

# -- Fibonacci Retracement Levels --
FIB_LEVEL_382   = 0.382
FIB_LEVEL_500   = 0.500
FIB_LEVEL_618   = 0.618
FIB_LEVEL_786   = 0.786        # Beyond this = impulse invalidated

# Entry zone: price must be within this tolerance of a fib level (points)
...
# -- Bounce Confirmation --
BOUNCE_MIN_CANDLES    = 2      # Min candles near fib level before entry
BOUNCE_MAX_CANDLES    = 8      # Max candles to wait for bounce (else stale)
...
# =============================================================
#  SECTION 3 - FIBONACCI SIGNAL ENGINE (FIXED)
# =============================================================
...
def compute_impulse_leg(candles):
    """
    Identify the morning impulse leg from candles between 9:15 and IMPULSE_END.
    
    FIXES vs original:
    1. Lowered min candles from 3 to 2 (9:15-9:45 = 6 candles on 5min, but gaps possible)
    2. Added debug logging for every filter that rejects
    3. Handles both epoch and string timestamps
    4. Uses <= for end time (includes the 9:40 candle for 9:45 end)
    5. Relaxed MIN_IMPULSE_POINTS with fallback
    """
    today = ist_now().date()
    impulse_candles = []
    ...
    for c in candles:
        ...
        try:
            ...
            try:
                dt = datetime.datetime.strptime(ts.replace("T", " ").split(".")[0], "%Y-%m-%d %H:%M:%S")
            except:
                logging.warning("IMPULSE SCAN | Unparseable timestamp: '%s'" % ts)
                continue
        ...
        # Include candles from 9:15 up to (but not including) impulse_end
        # For 5-min candles with end=9:45, this captures: 9:15, 9:20, 9:25, 9:30, 9:35, 9:40
    ...
    if len(impulse_candles) < 2:
        ...
        if today_candle_count > 0:
            # Log first and last today's candle timestamps for debugging
            today_ts = []
            for c in candles:
                ts = c.get("ts", "")
                try:
    ...
    if impulse_size > MAX_IMPULSE_POINTS:
        logging.warning("IMPULSE SCAN | FAILED: Size %.1f > max %d pts (exhaustion)" % (impulse_size, MAX_IMPULSE_POINTS))
        return None
    ...
    if last_close > first_open and high_idx >= low_idx:
        ...
        # Tiebreaker: use close vs open
        if last_close > first_open:
            direction = "BULL"
        elif last_close < first_open:
            direction = "BEAR"
        else:
            # True doji - use high/low position as final tiebreaker
            if high_idx > low_idx:
                direction = "BULL"
            elif low_idx > high_idx:
                direction = "BEAR"
            else:
                logging.warning("IMPULSE SCAN | FAILED: Perfect doji impulse, no direction")
                return None
...
def _find_nearest_fib_level(price, fib_data):
    """
    Returns the nearest fib level name, value, and distance.
    """
    levels = [
...
def _check_bounce(candles, fib_level, direction, tolerance):
    """
    Check if recent candles show a bounce off the fib level.
    For BULL: price dipped near fib level and is now moving up (buying bounce).
    For BEAR: price rallied near fib level and is now moving down (selling bounce).

    Returns (is_bouncing, candles_at_level, bounce_strength).
    """
    if len(candles) < BOUNCE_MIN_CANDLES:
        return False, 0, 0.0

    # Look at recent candles for bounce pattern
    ...
    for c in recent:
        ...
        if direction == "BULL":
            # Price should touch/approach fib level from above, then bounce up
            ...
            # Price should touch/approach fib level from below, then bounce down
    ...
    # Confirm bounce direction with last 2 candles
    last = candles[-1]
    prev = candles[-2]

    if direction == "BULL":
        # Last candle should be green (close > open) and closing above fib level
        bounce_confirmed = (last["close"] > last["open"] and
                           last["close"] > fib_level and
                           last["low"] >= fib_level - tolerance)
        # Strength: how far above fib level the close is
        bounce_strength = (last["close"] - fib_level) / tolerance if tolerance > 0 else 0
    else:
        # Last candle should be red (close < open) and closing below fib level
        bounce_confirmed = (last["close"] < last["open"] and
                           last["close"] < fib_level and
...
def compute_signal(candles, fib_data):
    """
    Fibonacci retracement signal engine.
    Waits for price to pull back to a key fib level, confirm bounce, then enter.
    """
    ...
    # -- CHECK IF IMPULSE IS INVALIDATED --
    # If price retraced beyond 78.6%, the impulse structure is broken
    fib_786 = fib_data["fib_786"]
    ...
    # -- CHECK IF PRICE ALREADY EXTENDED PAST SWING (no retracement happened) --
    ...
    # ADX filter - need trending market for fib to work
    ...
    # -- CHECK IF PRICE IS IN THE FIB ZONE --
    ...
    # Filter today's candles only (post-impulse)
    today = ist_now().date()
    ...
    if direction == "BULL":
        ...
        # Nifty target at extension
        nifty_target = fib_data["ext_1272"]
    else:
        # SL just above the next fib level up or 78.6%
        if fib_name == "38.2%":
            fib_sl_level = fib_data["fib_500"] + SL_BUFFER_POINTS
        elif fib_name == "50.0%":
            fib_sl_level = fib_data["fib_618"] + SL_BUFFER_POINTS
    ...
    # -- CONFIDENCE SCORING --
    score = 0
    score += 1  # Fib bounce confirmed (already passed)
    score += 1 if fib_name == "61.8%" else 0  # Golden ratio bonus
