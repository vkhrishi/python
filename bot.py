...
#    • OCO placed as broker-side safety net (backup)
#
#  STRATEGY:
...
# —— WebSocket monitor ——
WS_TICK_LOG_INTERVAL = 30   # Log LTP every 30 seconds (avoid log spam)
WS_HEARTBEAT_SEC     = 60   # Log heartbeat every 60 seconds
TRAILING_SL_ENABLED  = False
TRAILING_SL_TRIGGER  = 0.5  # Start trailing after 50% of target reached
TRAILING_SL_STEP     = 0.3  # Trail SL to lock 30% of profit
...
def compute_signal(candles, orb):
    ...
    details.update({
        ...
        "rr_ratio": f"1:{TARGET_RR_RATIO:.1f}",
...
def get_exchange_token(groww, trading_symbol):
    ...
    try:
        ...
        logging.warning(f"Instrument lookup failed for {trading_symbol}: {e}")
    return None
...
def place_exit_order(groww, symbol, qty, reason):
    if PAPER_TRADE:
        logging.info(f"?? PAPER EXIT | {symbol} | {reason}")
        return True
    try:
        groww.place_order(
            trading_symbol=symbol, quantity=qty, validity=groww.VALIDITY_DAY,
        ...
        logging.info(f"EXIT | {symbol} | {reason}")
        return True
    except Exception as e:
...
# -------------------------------------------------------------
#  SECTION 10 — WEBSOCKET MONITOR (core new feature)
# -------------------------------------------------------------

def write_monitor_pid():
    """Write current PID so cron knows monitor is running."""
    with open(MONITOR_PID, "w") as f:
        f.write(str(os.getpid()))

def clear_monitor_pid():
    """Remove PID file when monitor exits."""
    try: os.remove(MONITOR_PID)
    except: pass

def is_monitor_running():
    """Check if a WebSocket monitor process is already alive."""
    try:
...
def run_websocket_monitor(groww, state):
    ...
    trailing_sl  = sl_price  # Will be updated if trailing is enabled

    logging.info(f"--- WS MONITOR START ---")
    logging.info(f"  Symbol: {symbol} | Qty: {qty}")
    logging.info(f"  Entry: Rs.{entry_premium} | SL: Rs.{sl_price} | Target: Rs.{target_price}")
    logging.info(f"  Risk: Rs.{risk_premium * qty:.0f} | Reward: Rs.{target_premium * qty:.0f}")

    # Write PID so cron won't interfere
    write_monitor_pid()

    # Get exchange_token for WebSocket subscription
    exchange_token = get_exchange_token(groww, symbol)
    if not exchange_token:
        logging.error(f"Cannot get exchange_token for {symbol} — falling back to polling")
    ...
    def on_tick(meta):
        ...
        nonlocal sl_price, target_price
        ...
        try:
            ...
            if not ltp_data:
                return
            # Navigate the nested response: {"ltp": {"NSE": {"FNO": {"token": {"ltp": x}}}}}
            nse_data = ltp_data.get("ltp", ltp_data)
            if isinstance(nse_data, dict):
                nse = nse_data.get("NSE", nse_data)
                if isinstance(nse, dict):
                    ...
                    if isinstance(fno, dict):
                        ...
                        if isinstance(token_data, dict):
                            ...
                            current_ltp = float(token_data) if token_data else 0
                    else:
                        current_ltp = 0
                else:
                    current_ltp = 0
            ...
            logging.warning(f"WS tick parse error: {e}")
            return

        monitor_state["last_ltp"] = current_ltp

        # —— Periodic logging (avoid spam) ——
        if now - monitor_state["last_log_time"] >= WS_TICK_LOG_INTERVAL:
            pnl = (current_ltp - entry_premium) * qty
            logging.info(f"  ?? LTP: Rs.{current_ltp:.1f} | P&L: Rs.{pnl:+.0f} | "
                        f"SL: Rs.{monitor_state['trailing_sl']:.1f} | T: Rs.{target_price:.1f}")
            monitor_state["last_log_time"] = now

        # —— Heartbeat ——
        if now - monitor_state["last_heartbeat"] >= WS_HEARTBEAT_SEC:
            logging.info(f"  ?? Monitor alive | LTP: Rs.{current_ltp:.1f}")
        ...
        if now_ist.hour > SQUAREOFF_HOUR or (now_ist.hour == SQUAREOFF_HOUR and now_ist.minute >= SQUAREOFF_MIN):
            ...
            _execute_exit(groww, state, trade, feed, instrument_list,
                         current_ltp, pnl, f"EOD SQUAREOFF Rs.{pnl:+.0f}", monitor_state)
            return
        ...
        if TRAILING_SL_ENABLED and current_ltp > monitor_state["highest_ltp"]:
            ...
            if profit_so_far >= trigger_profit:
                ...
                if new_sl > monitor_state["trailing_sl"]:
                    ...
                    logging.info(f"  ?? TRAILING SL: Rs.{old_sl:.1f} ? Rs.{new_sl:.1f}")
        ...
        if current_ltp >= target_price:
            pnl = (current_ltp - entry_premium) * qty
            _execute_exit(groww, state, trade, feed, instrument_list,
                         current_ltp, pnl, f"? TAKE PROFIT Rs.{pnl:+.0f}", monitor_state)
            return
        ...
        if current_ltp <= active_sl:
            pnl = (current_ltp - entry_premium) * qty
            _execute_exit(groww, state, trade, feed, instrument_list,
                         current_ltp, pnl, f"?? STOP LOSS Rs.{pnl:+.0f}", monitor_state)
            return

    def _execute_exit(groww, state, trade, feed, instrument_list,
                      current_ltp, pnl, reason, mstate):
        """Execute exit order and clean up."""
        if mstate["exited"]:
            return
        mstate["exited"] = True

        logging.info(f"--- EXIT TRIGGERED ---")
        logging.info(f"  {reason}")
        logging.info(f"  LTP: Rs.{current_ltp:.1f} | Entry: Rs.{entry_premium}")

        # Place exit order
        exit_ok = place_exit_order(groww, symbol, qty, reason)
        ...
        logging.info(f"  Daily P&L: Rs.{state['daily_pnl_rupees']:+.0f}")
        logging.info(f"--- WS MONITOR END ---")

    # —— Signal handler for clean shutdown ——
    def handle_shutdown(signum, frame):
        logging.info("WS Monitor: Shutdown signal received")
        if not monitor_state["exited"]:
            # Try to get final LTP and exit
            try:
                ltp_data = feed.get_ltp()
                nse = ltp_data.get("ltp", {}).get("NSE", {}).get("FNO", {})
                token_data = nse.get(exchange_token, {})
                final_ltp = float(token_data.get("ltp", 0)) if isinstance(token_data, dict) else 0
            except:
                final_ltp = monitor_state.get("last_ltp") or entry_premium
            if final_ltp > 0:
                pnl = (final_ltp - entry_premium) * qty
                _execute_exit(groww, state, trade, feed, instrument_list,
                             final_ltp, pnl, f"SHUTDOWN Rs.{pnl:+.0f}", monitor_state)
        clear_monitor_pid()
        sys.exit(0)
    ...
    logging.info(f"  Subscribing to {symbol} (token: {exchange_token})...")
    ...
    try:
        ...
        logging.error(f"WS Monitor crashed: {e}")
...
def _fallback_polling_monitor(groww, state, trade):
    ...
    logging.info(f"POLLING MONITOR | {symbol} | SL:{sl_price} | T:{target_price}")
    write_monitor_pid()
    ...
    try:
        while True:
            ...
            if now.hour > SQUAREOFF_HOUR or (now.hour == SQUAREOFF_HOUR and now.minute >= SQUAREOFF_MIN):
                ...
                pnl = (current_ltp - entry_premium) * qty
                place_exit_order(groww, symbol, qty, f"EOD SQUAREOFF Rs.{pnl:+.0f}")
                trade["exited"] = True
                trade["exit_ltp"] = current_ltp
                trade["exit_pnl"] = round(pnl, 2)
            ...
            if int(time.time()) % 30 < 6:
                logging.info(f"  POLL | LTP:{current_ltp:.1f} | P&L:{pnl:+.0f} | SL:{sl_price} T:{target_price}")

            if current_ltp >= target_price:
                place_exit_order(groww, symbol, qty, f"? TP Rs.{pnl:+.0f}")
                trade["exited"] = True
                trade["exit_ltp"] = current_ltp
                trade["exit_pnl"] = round(pnl, 2)
            ...
            if current_ltp <= sl_price:
                place_exit_order(groww, symbol, qty, f"?? SL Rs.{pnl:+.0f}")
                trade["exited"] = True
                trade["exit_ltp"] = current_ltp
                trade["exit_pnl"] = round(pnl, 2)
        ...
        logging.error(f"Polling monitor error: {e}")
    finally:
        clear_monitor_pid()
...
def main():
    ...
    if is_monitor_running():
        logging.info(f"[{now.strftime('%H:%M')}] Monitor active. Skip scan.")
        return

    # —— Check if there's an unexited trade (monitor crashed?) ——
    trades = state.get("trades", [])
    ...
    logging.info(f"--- {mode_tag} SCAN | {now.strftime('%Y-%m-%d %H:%M:%S')} | "
                f"Trades:{state['trade_count']}/{MAX_TRADES_DAY} | "
                f"P&L:Rs.{state.get('daily_pnl_rupees',0):+.0f} ---")
    ...
    if orb is None:
        ...
        logging.info(f"ORB | H:{orb['orb_high']} L:{orb['orb_low']} R:{orb['orb_range']} "
                    f"GAP:{orb['gap_direction']}({orb['gap_size']})")
    ...
    logging.info(f"  Signal:{signal_type} | Conf:{confidence} | {d.get('trigger','—')}")
    logging.info(f"  Close:{d.get('close','—')} | ORB:{d.get('orb_high','—')}/{d.get('orb_low','—')} | "
                f"Gap:{d.get('gap_direction','—')}({d.get('gap_size','—')})")
    logging.info(f"  ADX:{d.get('adx','—')} RSI:{d.get('rsi','—')} VWAP:{d.get('vwap','—')} | "
                f"Bull:{d.get('bull_score','—')} Bear:{d.get('bear_score','—')}")
    if d.get("fvg_top"):
    ...
    logging.info(f"{tag} | {signal_type} {confidence} | {symbol}")
    logging.info(f"   E:Rs.{entry_premium} SL:Rs.{sl_price} T:Rs.{target_price} RR:1:{rr:.1f}")
    logging.info(f"   {d.get('trigger','')} | Score:{d.get('score','')}")
...
def _spawn_monitor():
    ...
    logging.info(f"Spawning monitor: {' '.join(cmd)}")
    ...
    logging.info(f"Monitor spawned: PID {proc.pid}")
