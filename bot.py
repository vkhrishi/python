...
NO_TRADE_AFTER_MIN   = 00   # Strict: no entries after 11:30
SQUAREOFF_HOUR       = 15
SQUAREOFF_MIN        = 10

# ── Market regime ──
MIN_ADX_FOR_TRADE    = 18   # Reduced from 20 — morning ADX often 15-19
ADX_LEN              = 14
ATR_LEN              = 14
RSI_LEN              = 14
EMA_FAST             = 9
...
LIMIT_BUY_BUFFER      = 1.02
LIMIT_SELL_BUFFER     = 0.98

MIN_HOLD_TIME_SEC     = 180   # 3 min hold
ENABLE_SOFT_SL        = True
ENABLE_CANDLE_EXIT    = True

# ===== ENTRY FILTERS =====
REQUIRE_2_CANDLE_CONFIRMATION = False
REQUIRE_FVG_RETEST            = False  # set True for sniper mode
...
def compute_signal(candles, orb):
	...
	# 2-candle breakout confirmation
    if REQUIRE_2_CANDLE_CONFIRMATION and len(closes) >= 2:
        prev_close = closes[-2]
    if last_close > breakout_high and prev_close <= breakout_high:
        return {"signal": "NO_TRADE", "details": {"reason": "No 2-candle confirmation"}}
    if last_close < breakout_low and prev_close >= breakout_low:
        return {"signal": "NO_TRADE", "details": {"reason": "No 2-candle confirmation"}}
    # -- 2-candle breakout confirmation (FIXED) --
    if REQUIRE_2_CANDLE_CONFIRMATION and len(closes) >= 2:
        prev_close = closes[-2]

        # Bullish: previous candle should already be above breakout (strong continuation)
        if last_close > breakout_high:
            if prev_close < (breakout_high - 2):   # <-- your +filter added here
                return {"signal": "NO_TRADE", "details": {"reason": "Weak breakout (no 2-candle strength)"}}

        # Bearish: previous candle should already be below breakout
        if last_close < breakout_low:
            if prev_close > (breakout_low + 2):   # <-- symmetric condition
                return {"signal": "NO_TRADE", "details": {"reason": "Weak breakdown (no 2-candle strength)"}}
...
def place_entry_order(groww, symbol, qty, txn, ltp):
    if PAPER_TRADE:
    ...
    try:
        if USE_LIMIT_ORDERS:
            price = round(ltp * LIMIT_BUY_BUFFER, 1)
        ...
        res = groww.place_order(
            ...
            order_reference_id=f"SNP{ist_now().strftime('%H%M%S')}"
        )

        oid = res.get("groww_order_id", "N/A")
        logging.info(f"ENTRY | {symbol} | Price:{price} | ID:{oid}")
        return oid
...
def monitor_and_exit(groww, state):
    ...
    last_trade = trades[-1]
    if last_trade.get("exited", False):
        return False
    ...
    entry_time = datetime.datetime.strptime(last_trade["time"], "%Y-%m-%d %H:%M:%S")

    # ? MIN HOLD TIME
    if (ist_now() - entry_time).total_seconds() < MIN_HOLD_TIME_SEC:
        logging.info("Hold period active � skipping exit")
        return False
    ...
    # ? CANDLE CLOSE EXIT ONLY
    now = ist_now()
    if ENABLE_CANDLE_EXIT and now.second < 50:
        return False
    ...
    # ? SOFT STOP LOSS
        ...
        if ENABLE_SOFT_SL:
            if last_trade.get("sl_hit_once"):
                exit_reason = f"CONFIRMED SL Rs.{pnl_total:+.0f}"
            else:
                last_trade["sl_hit_once"] = True
                logging.info("Soft SL hit once � waiting confirmation")
                return False
        else:
    ...
    try:
        if USE_LIMIT_ORDERS:
            price = round(current_ltp * LIMIT_SELL_BUFFER, 1)
            order_type = "LIMIT"
        else:
            price = None
            order_type = "MARKET"
        ...
        logging.info(f"EXIT | {symbol} | {exit_reason}")
    ...
    last_trade["exit_pnl"] = pnl_total
    last_trade["exit_reason"] = exit_reason

    state["daily_pnl_rupees"] += pnl_total
    save_state(state)
...
def main():
    ...
    oid = place_entry_order(groww, symbol, qty, groww.TRANSACTION_TYPE_BUY, option_ltp)
    if not oid:
        return
