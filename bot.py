...
# Low premium (Rs.50-100):   SL = 15%
# Mid premium (Rs.100-250):  SL = 12%
...
MAX_RISK_RUPEES       = CAPITAL_RUPEES * RISK_PER_TRADE_PCT / 100  # Rs.3000
MAX_DAILY_LOSS_RUPEES = MAX_RISK_RUPEES  # Same (1 trade/day)

# -- Hold time and catastrophic SL --
MIN_HOLD_MINUTES      = 8       # No normal SL exit before 8 minutes
...
def compute_signal(candles, orb):
    ...
    # =========================================================
    #  NEW FILTER 1: BREAKOUT CANDLE STRENGTH
    #  Reject weak/doji candles — no conviction behind the move
    # =========================================================
    ...
    if last_range > 0:
        ...
        if body_ratio < 0.4:
            details["reason"] = "Weak breakout candle: body %.0f%% of range (need 40%%+)" % (body_ratio * 100)
            return {"signal": "NO_TRADE", "details": details}

    # =========================================================
    #  NEW FILTER 2: CANDLE DIRECTION ALIGNMENT
    #  BULL needs green candle, BEAR needs red candle
    # =========================================================
    ...
    #  Breakout candle volume must be >= 1.0x 20-candle average
    # =========================================================
    ...
    if avg_vol > 0:
        ...
        if vol_ratio < 1.0:
            details["reason"] = "Low volume breakout: %.1fx avg (need 1.0x+)" % vol_ratio
            return {"signal": "NO_TRADE", "details": details}

    # =========================================================
    #  NEW FILTER 4: CONSECUTIVE DIRECTIONAL CANDLES
    #  Need 2/3 last candles closing in breakout direction
    # =========================================================
    if len(candles) >= 3:
        if direction == "BULL":
    ...
    # =========================================================
    #  NEW FILTER 5: LATE ENTRY HIGHER CONFLUENCE
    #  After 10:30 require 4/6 score instead of 3/6
    # =========================================================
    ...
    if is_late:
        ...
        if score < min_late_score:
            details["reason"] = "Late entry (%s): need %d/6, got %d/6" % (
                now.strftime("%H:%M"), min_late_score, score)
            return {"signal": "NO_TRADE", "details": details}
    else:
        details["late_entry"] = False

    # =========================================================
    #  NEW FILTER 6: ATR BREAKOUT DISTANCE CHECK
    #  Must break ORB by 0.3x-2.0x ATR (not too shallow, not chasing)
    # =========================================================
    if atr is not None and atr > 0:
        ...
        if atr_multiple < 0.3:
            details["reason"] = "Breakout too shallow: %.2fx ATR (need 0.3x+)" % atr_multiple
            return {"signal": "NO_TRADE", "details": details}
        if atr_multiple > 2.0:
