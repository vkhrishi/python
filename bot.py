...
def compute_signal(candles, orb):
    ...
    if last_range > 0:
        ...
        if body_ratio < 0.4:
            details["reason"] = "Weak breakout candle: body " + str(round(body_ratio * 100)) + " pct of range (need 40 pct+)"
            return {"signal": "NO_TRADE", "details": details}
    ...
    if avg_vol > 0:
        ...
        if vol_ratio < 1.0:
            details["reason"] = "Low volume breakout: " + str(round(vol_ratio, 1)) + "x avg (need 1.0x+)"
            return {"signal": "NO_TRADE", "details": details}
    ...
    if len(candles) >= 3:
        ...
        if consec < 2:
            details["reason"] = "Weak momentum: only " + str(consec) + "/3 candles in direction"
            return {"signal": "NO_TRADE", "details": details}
    ...
    if is_late:
        ...
        if score < min_late_score:
            details["reason"] = "Late entry (" + now.strftime("%H:%M") + "): need " + str(min_late_score) + "/6, got " + str(score) + "/6"
            return {"signal": "NO_TRADE", "details": details}
    else:
    ...
    if atr is not None and atr > 0:
        ...
        if atr_multiple < 0.3:
            details["reason"] = "Breakout too shallow: " + str(round(atr_multiple, 2)) + "x ATR (need 0.3x+)"
            return {"signal": "NO_TRADE", "details": details}
        if atr_multiple > 2.0:
            details["reason"] = "Overextended: " + str(round(atr_multiple, 2)) + "x ATR (max 2.0x)"
            return {"signal": "NO_TRADE", "details": details}
