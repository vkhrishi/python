...
IMPULSE_END_HOUR     = 9    # 30-min opening-range impulse (cleaner single-leg)
IMPULSE_END_MIN      = 45
...
BOUNCE_MIN_CLOSE_POS = 0.6    # Bounce candle must close in top 40% of range (BULL) / bottom 40% (BEAR): rejects spike-and-fade
...
# the latest candle missed valid setups ("no bounce confirmation yet").
BOUNCE_LOOKBACK      = 3

# -- Entry quality filters --
MAX_CHASE_ATR        = 0.8    # Reject if price has run > this x ATR past the zone edge (don't chase a faded bounce)
MIN_CONFLUENCE_SCORE = 4      # Min confluence score (of 7) required to enter
MIN_CONFLUENCE_SCORE_LATE = 5 # Stricter floor for entries after 11:00
...
RR_RATIO          = 2.0       # Target = SL x 2.0 (reachable TP on option premiums; trailing locks the rest)
MIN_RR_RATIO      = 1.5       # Reject if below 1:1.5 (was 1:2; matches the lower, more-reachable target)
...
# -- Cost model (round-trip costs deducted from backtest P&L) --
...
SLIPPAGE_PER_SIDE_PCT = 0.3     # % premium slippage per side (entry+exit)
...
# -- Volume filter (replaces the old ADX trend gate) --
MIN_VOLUME_RATIO     = 0.8     # Entry bar volume must be >= this x recent avg; no-op if feed has no volume
VOLUME_FILTER_BARS   = 20      # Lookback bars for the volume average

# -- Higher-timeframe (HTF) trend filter --
# Confirm the 5M entry agrees with the 15M trend before trading. Pure GATE:
# it can only BLOCK a trade that fights the 15M trend, never change the
# Fibonacci logic, sizing, SL or TP. If the 15M trend is undecided we allow the
# trade (we don't add a new way to miss setups). The 15M series is resampled
# from the cached 5M candles, so live and backtest share one derivation with no
# extra API call.
HTF_TREND_ENABLED = True   # Gate counter-trend entries: only take a fib trade
                           # that agrees with the 15M trend (blocks range-chop losses)
HTF_INTERVAL_MIN  = 15
HTF_EMA_FAST      = 9
HTF_EMA_SLOW      = 21
...
WS_HEARTBEAT_SEC     = 60
TRAILING_SL_ENABLED  = True    # Enabled: SL ratchets up with price to lock in open profit
TRAILING_SL_TRIGGER  = 0.375   # Start trailing at 0.75R profit (0.375 x 1:2 target) - lock gains earlier
TRAILING_SL_STEP     = 0.60    # Lock 60% of open profit - protect winners harder
...
def resample_to_htf(candles, minutes=15):
    """Aggregate chronologically-sorted 5m candle dicts into higher-timeframe
    candles grouped by clock boundary within each trading day. Used for the 15M
    trend filter so live and backtest share one derivation (no extra API call)."""
    buckets = {}
    order = []
    for c in candles:
        try:
            ...
            continue
        bucket_min = dt.minute - (dt.minute % minutes)
        key = (dt.year, dt.month, dt.day, dt.hour, bucket_min)
...
def compute_impulse_and_fibs(candles, target_date=None):
    """
...
def _detect_bounce(candles, idx, direction, entry_upper, entry_lower):
    """Return (bounce_type, body_pct) if the candle at `idx` confirms a fib-zone
    bounce in `direction`, else (None, None). `idx` is a (negative) index into
    `candles`. Factored out so the scanner can look back over several recent
    candles instead of only the latest one."""
    c = candles[idx]
    o = c["open"]
    cl = c["close"]
    h = c["high"]
    ...
    if direction == "BULL":
        ...
        if cl > o and rng > 0:
            body_pct = body / rng
            close_pos = (cl - l) / rng
            # Strong bounce: bullish body >= threshold AND closing near its high
            # (buyers in control at the close, not a spike-and-fade)
            if body_pct >= BOUNCE_MIN_BODY_PCT and close_pos >= BOUNCE_MIN_CLOSE_POS:
                return "BULLISH_ENGULF", round(body_pct * 100, 1)
            # Hammer pattern (long lower wick, small body at top)
            lower_wick = o - l
            upper_wick = h - cl
            if lower_wick > body * 2 and upper_wick < body:
                return "HAMMER", None
        # Previous candle was in zone, this one closed above its high
        prev_idx = idx - 1
        if -prev_idx <= len(candles):
            prev = candles[prev_idx]
            if prev["low"] <= entry_upper and cl > prev["high"]:
                return "PREV_CANDLE_BOUNCE", None
    else:  # BEAR
        body = o - cl
        if cl < o and rng > 0:
            body_pct = body / rng
            close_pos = (h - cl) / rng
            if body_pct >= BOUNCE_MIN_BODY_PCT and close_pos >= BOUNCE_MIN_CLOSE_POS:
                return "BEARISH_ENGULF", round(body_pct * 100, 1)
            upper_wick = h - o
            lower_wick = cl - l
            if upper_wick > body * 2 and lower_wick < body:
                return "SHOOTING_STAR", None
        prev_idx = idx - 1
        if -prev_idx <= len(candles):
            prev = candles[prev_idx]
            if prev["high"] >= entry_lower and cl < prev["low"]:
                return "PREV_CANDLE_BOUNCE", None
    return None, None


def check_fib_retracement(candles, fib_data, now=None):
    """
    ...
    # Latched invalidation: once price has closed beyond the 78.6% level earlier
    # today, the setup is dead for the rest of the session. Without this a brief
    # tick back into the golden pocket re-armed a broken thesis and produced
    # repeat losing entries (07-Jul: invalidated 12:39 -> re-entered PE at 12:40).
    if fib_data.get("invalidated"):
        details["reason"] = "Fib invalidated earlier today - setup dead for the day"
    ...
    # Volume filter - need sufficient participation. NIFTY spot has no volume,
    # so fetch_candles borrows it from the front-month future (see
    # _enrich_volume_from_futures). Safe no-op if volume is still unavailable.
    vfb = min(VOLUME_FILTER_BARS, len(volumes))
    recent_vol = [v for v in volumes[-vfb:] if v > 0]
    avg_vol = sum(recent_vol) / len(recent_vol) if recent_vol else 0
    cur_vol = volumes[-1] if volumes else 0
    details["vol_now"] = int(cur_vol)
    details["vol_avg"] = int(avg_vol)
    if avg_vol > 0 and cur_vol > 0 and cur_vol < avg_vol * MIN_VOLUME_RATIO:
        details["reason"] = "Low volume: %d < %.2fx avg %d" % (cur_vol, MIN_VOLUME_RATIO, avg_vol)
        return {"signal": "NO_TRADE", "details": details}
    ...
    # STEP 2: Bounce confirmation — scan the last BOUNCE_LOOKBACK candles
    # (newest first), not just candles[-1]. The retracement bounce frequently
    # prints 1-3 candles before the scanner runs again; only inspecting the
    # latest candle returned "no bounce confirmation yet" even though a valid
    # bounce had already occurred. This matches the multi-candle zone-touch
    # window used above.
    bounce_confirmed = False
    lookback = min(BOUNCE_LOOKBACK, len(candles))
    for age in range(lookback):
        bounce_type, body_pct = _detect_bounce(
            candles, -1 - age, direction, entry_upper, entry_lower)
        if bounce_type:
    ...
    # STEP 2.5: Don't chase — reject if price has already run too far past the
    # zone. A confirmed bounce that has extended > MAX_CHASE_ATR beyond the zone
    # edge is usually a late/faded entry with poor risk-reward.
    if atr is not None and atr > 0:
        if direction == "BULL" and last_close > entry_upper + MAX_CHASE_ATR * atr:
            details["reason"] = "Chasing: close %.1f > zone top %.1f + %.2fx ATR" % (
                last_close, entry_upper, MAX_CHASE_ATR)
            return {"signal": "NO_TRADE", "details": details}
        if direction == "BEAR" and last_close < entry_lower - MAX_CHASE_ATR * atr:
            details["reason"] = "Chasing: close %.1f < zone bottom %.1f - %.2fx ATR" % (
                last_close, entry_lower, MAX_CHASE_ATR)
            return {"signal": "NO_TRADE", "details": details}
    ...
    is_late = now.hour >= 11
    if is_late:
        min_score = MIN_CONFLUENCE_SCORE_LATE  # Higher bar for late entries
...
def calc_sl_tp(entry_price, qty, fib_sl_distance=None):
    ...
    if fib_sl_distance and fib_sl_distance > 0:
        ...
        # ITM_OFFSET=100 means we buy ITM, delta ~0.6, so option moves ~60% of index
        sl_drop = fib_sl_distance * 0.60  # delta scaling for ITM options
...
_FUT_SYM_CACHE = {"date": None, "symbol": None}

def _resolve_nifty_future(groww):
    """Trading symbol of the nearest-expiry NIFTY future (cached per day).
    Used only to borrow real traded volume for the volume filter, since the
    NIFTY spot index reports none. Returns None if it can't be resolved."""
    today = ist_now().strftime("%Y-%m-%d")
    if _FUT_SYM_CACHE["date"] == today and _FUT_SYM_CACHE["symbol"]:
        return _FUT_SYM_CACHE["symbol"]
    sym, best_exp = None, None
    try:
        df = groww._load_instruments()
        mask = ((df["underlying_symbol"] == "NIFTY") &
                (df["exchange"] == "NSE") &
                (df["segment"] == "FNO") &
                (df["instrument_type"].astype(str).str.upper().str.contains("FUT", na=False)))
        ...
        for _, row in df[mask].iterrows():
            ...
            if exp_date < today_d:
                continue
            if best_exp is None or exp_date < best_exp:
                best_exp, sym = exp_date, str(row["trading_symbol"])
        if sym:
            logging.info("NIFTY future for volume: %s (exp %s)" % (sym, best_exp))
        else:
            logging.warning("No NIFTY future found for volume filter")
    except Exception as e:
        logging.warning("Futures symbol resolve failed: %s" % str(e))
    _FUT_SYM_CACHE["date"] = today
    _FUT_SYM_CACHE["symbol"] = sym
    return sym

def _fetch_nifty_future_volume_map(groww, start_dt, end_dt):
    """Return {ts: volume} from the nearest-expiry NIFTY future 5-min candles.
    Empty dict on any failure so callers degrade to a no-op volume filter."""
    sym = _resolve_nifty_future(groww)
    if not sym:
        return {}
    try:
        res = groww.get_historical_candle_data(
            trading_symbol=sym, exchange="NSE", segment="FNO",
            start_time=start_dt, end_time=end_dt, interval_in_minutes=5)
        raw = res.get("candles", []) if isinstance(res, dict) else []
        return {c["ts"]: c["volume"] for c in _parse_candles(raw) if c.get("ts")}
    except Exception as e:
        logging.warning("Futures candle fetch failed (%s): %s" % (sym, str(e)))
        return {}

def _enrich_volume_from_futures(groww, candles, start_dt, end_dt):
    """Overwrite the empty NIFTY spot volume with the front-month future's
    volume, matched by timestamp. The NIFTY index (CASH) has no traded volume,
    so the volume filter borrows it from the liquid future. Safe no-op on any
    failure (volume stays 0 and the filter simply doesn't gate). Backtest note:
    only the current front-month contract is used, so days before it began
    trading (across a monthly rollover) stay un-enriched."""
    if not candles:
        return
    try:
        vol_map = _fetch_nifty_future_volume_map(groww, start_dt, end_dt)
        if not vol_map:
            return
        n = 0
        for c in candles:
            v = vol_map.get(c.get("ts"))
            if v:
                c["volume"] = v
                n += 1
        if n:
            logging.info("Futures volume: enriched %d/%d candle(s)" % (n, len(candles)))
    except Exception as e:
        logging.warning("Volume enrichment failed: %s" % str(e))
...
def fetch_candles(groww):
    ...
    try:
        ...
        if raw:
            ...
            cache["date"] = today
            _enrich_volume_from_futures(
                groww, cache["candles"],
                now.strftime("%Y-%m-%d 09:15:00"), end_dt)
            _save_candles(cache)
...
def run_websocket_monitor(groww, state):
    ...
    def on_tick(meta):
        ...
        # TRAILING SL — live from the very first tick so early spikes are locked
        # in (trade 1 peaked +4.5% only ~3 min in, inside the old hold window, and
        # gave it all back). Trailing only ever ratchets the stop UP, so making it
        # always-on can never widen risk.
        ...
        # STAGNATION EXIT: cut dead trades to stop theta bleed (only after hold)
        ...
        # HARD STOP LOSS — trailed stop (never below the planned SL) plus the
        # catastrophic backstop. Both live from the first tick; the hold window
        # no longer suppresses capital protection.
...
def _fallback_polling_monitor(groww, state, trade):
    ...
    try:
        while True:
            ...
            # TRAILING SL — live from the first poll so early spikes are locked in
            # (trailing only ratchets the stop UP, so it can never widen risk).
            ...
            # HARD STOP LOSS — trailed stop (never below the planned SL) plus the
            # catastrophic backstop, both live from the first poll.
...
# =============================================================
#  SECTION 10b - BACKTEST HARNESS (FIBONACCI RETRACEMENT)
# =============================================================
#  Replays the live signal engine (compute_impulse_and_fibs +
#  check_fib_retracement + calc_sl_tp) over historical NIFTY index
...
#   Real results differ because of theta decay, IV shifts (vega) and
#   delta drift (gamma). Treat the output as a DIRECTIONAL edge
#   check, not a precise P&L forecast. Confirm with paper trading.
# =============================================================

BT_DELTA          = 0.50   # assumed delta of the ITM-by-ITM_OFFSET option
...
def fetch_history_nifty(groww, days=30):
    ...
    try:
        ...
        _enrich_volume_from_futures(groww, candles, start_dt, end_dt)
...
def _simulate_fib_trade(direction, entry_spot, fib_sl_distance, forward):
    ...
    sl_price, target_price, sl_drop, tp_rise, sl_pct, rr = calc_sl_tp(
        entry_premium, qty, fib_sl_distance)
    if rr < MIN_RR_RATIO:
        return None

    rt_cost = estimate_roundtrip_cost(entry_premium, qty)
    ...
    for i, c in enumerate(forward):
        ...
        # HARD STOP LOSS first (conservative) — trailing_sl starts at the planned
        # SL and only ratchets up; it and the catastrophic backstop are live from
        # the first candle (no hold-window suppression, mirrors the live monitor).
...
def run_backtest_fib(days=30):
    """Backtest the Fibonacci-retracement strategy over the last `days` days."""
    groww = login()
    ...
    scan_start = datetime.time(IMPULSE_END_HOUR, IMPULSE_END_MIN)   # after impulse window
    ...
    for day in days_seen:
        ...
        days_total += 1
        fib_data = compute_impulse_and_fibs(candles, target_date=d0)
        if not fib_data:
            continue
        days_with_fib += 1
        direction = fib_data["direction"]
        ...
        for c in candles:
            ...
            res = check_fib_retracement(candles[:idx + 1], fib_data, now=dt)
            ...
            if HTF_TREND_ENABLED:
                htf = resample_to_htf(candles[:idx + 1], minutes=HTF_INTERVAL_MIN)
            ...
            sim = _simulate_fib_trade(direction, entry_spot, fib_sl_distance, forward)
            if sim:
                sim["day"] = day
    ...
    _print_bt_report_fib(trades, signals, days, approx=True, diag=diag)


def _expiry_for_date(d0):
    """Weekly NIFTY expiry (Tuesday) on or after a given date."""
    days_ahead = (NIFTY_EXPIRY_WEEKDAY - d0.weekday()) % 7
    return d0 + datetime.timedelta(days=days_ahead)


def _fetch_option_history(groww, symbol, day):
    """Fetch 5-min historical premium candles for an option symbol on one day."""
    start_dt = "%s 09:15:00" % day
    end_dt = "%s 15:30:00" % day
    try:
        res = groww.get_historical_candle_data(
            trading_symbol=symbol, exchange="NSE", segment="FNO",
            start_time=start_dt, end_time=end_dt, interval_in_minutes=5)
        raw = res.get("candles", []) if isinstance(res, dict) else []
        return _parse_candles(raw)
    except Exception as e:
        logging.warning("Option history failed for %s: %s" % (symbol, str(e)))
        return []


def _simulate_premium_trade(entry_premium, fib_sl_distance, prem_forward):
    """Simulate a long-option trade on REAL premium candles (option OHLC).
    Direction-agnostic: we BOUGHT the option, so profit = premium rising.
    Mirrors the live monitor exits exactly (no delta/theta approximation)."""
    qty = LOT_SIZE * LOTS_TO_TRADE
    sl_price, target_price, sl_drop, tp_rise, sl_pct, rr = calc_sl_tp(
        entry_premium, qty, fib_sl_distance)
    if rr < MIN_RR_RATIO:
    ...
    for i, c in enumerate(prem_forward):
        minutes_held = (i + 1) * 5     # 5-min candles
        low = c["low"]      # adverse for a long option
        high = c["high"]    # favourable
        close = c["close"]

        # During hold: catastrophic SL first (conservative), TP still active
        ...
        # Trailing-stop ratchet
        if TRAILING_SL_ENABLED and high > highest:
            highest = high
            open_profit = highest - entry_premium
            if open_profit >= tp_rise * TRAILING_SL_TRIGGER:
                new_sl = entry_premium + open_profit * TRAILING_SL_STEP
        ...
        # Stagnation
        if (STAGNATION_EXIT_ENABLED and minutes_held >= STAGNATION_EXIT_MIN
                and (close - entry_premium) < sl_drop * STAGNATION_MIN_PROFIT_RR):
...
def run_backtest_fib_real(days=30):
    """ACCURATE backtest using REAL historical option premiums.
    Resolves the option that would have been traded each signal day, fetches
    its actual 5-min premium candles, and simulates the trade on them."""
    groww = login()
    candles = fetch_history_nifty(groww, days=days)
    if len(candles) < 60:
        print("Not enough history fetched: %d candles" % len(candles))
    ...
    ts_index = {c["ts"]: i for i, c in enumerate(candles)}
    days_seen = sorted({c["ts"][:10] for c in candles})
    scan_start = datetime.time(IMPULSE_END_HOUR, IMPULSE_END_MIN)
    cutoff = datetime.time(NO_TRADE_AFTER_HOUR, NO_TRADE_AFTER_MIN)
    ...
    for day in days_seen:
        ...
        for c in candles:
            ...
            atm = get_atm_strike(entry_spot)
            strike = (atm - ITM_OFFSET) if opt_type == "CE" else (atm + ITM_OFFSET)
            expiry = _expiry_for_date(d0)
            symbol = build_symbol(strike, opt_type, expiry)

            prem_candles = _fetch_option_history(groww, symbol, day)
            if not prem_candles:
                skipped += 1
                break

            # entry premium = first option candle at/after the entry bar time
            ...
            sim = _simulate_premium_trade(entry_premium, fib_sl_distance, prem_forward)
            if sim:
                sim["day"] = day
                sim["dir"] = opt_type
                sim["conf"] = res.get("confidence")
    ...
    if skipped:
        print("Note: %d signal day(s) skipped (no option history available)" % skipped)
    diag = {"days_total": days_total, "days_with_fib": days_with_fib,
            "reason_tally": reason_tally}
    _print_bt_report_fib(trades, signals, days, approx=False, diag=diag)


def _print_diag_fib(diag):
    if not diag:
        return
    print("-" * 60)
...
def _print_bt_report_fib(trades, signals, days, approx=True, diag=None):
    print("\n" + "=" * 60)
    title = "APPROX" if approx else "REAL PREMIUM"
    print(" NIFTY BACKTEST — FIBONACCI (%s) | last %d days" % (title, days))
    if approx:
    ...
    if approx:
        ...
        print(" NOTE: uses REAL historical option premiums (accurate P&L).")
        print(" Live results still vary with slippage/spread and fill timing.")
    print("=" * 60)
...
def main():
    ...
    # MODE: backtest (approximated option P&L from index)
    if run_mode == "backtest":
        days = int(sys.argv[2]) if len(sys.argv) > 2 else 30
        run_backtest_fib(days=days)
        return

    # MODE: backtest_real (accurate — uses real historical option premiums)
    if run_mode == "backtest_real":
        days = int(sys.argv[2]) if len(sys.argv) > 2 else 30
        run_backtest_fib_real(days=days)
        return
    ...
    # Persist invalidation to disk so later cron scans (each a fresh process)
    # cannot re-arm a broken setup when price ticks back into the zone.
    if d.get("invalidated_now") and fib_data and not fib_data.get("invalidated"):
        fib_data["invalidated"] = True
        save_fib_levels(fib_data)
        logging.info("  Fib setup latched INVALIDATED for the rest of today")
    ...
    # Higher-timeframe (15M) trend gate. Only BLOCKS trades that fight the 15M
    # trend; it never alters the fib/sizing/SL/TP logic below. The 15M series is
    # resampled from the 5M candles already fetched above.
    if HTF_TREND_ENABLED:
        htf = resample_to_htf(candles, minutes=HTF_INTERVAL_MIN)
        htf_trend, hf, hs = compute_htf_trend(htf)
        want = fib_data["direction"]
        if htf_trend is None:
            logging.info("  HTF(%dm) trend undecided — allowing trade" % HTF_INTERVAL_MIN)
        else:
            logging.info("  HTF(%dm) trend: %s | EMA%d:%.1f EMA%d:%.1f" % (
                HTF_INTERVAL_MIN, htf_trend, HTF_EMA_FAST, hf, HTF_EMA_SLOW, hs))
            if htf_trend != want:
                logging.info("  -> Blocked: 5M %s vs %dm %s trend" % (
                    signal_type, HTF_INTERVAL_MIN, htf_trend))
                return
