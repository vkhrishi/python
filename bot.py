...
try:
    ...
    SERVER_IP = _req.get("https://api.ipify.org", timeout=3).text
    logging.info(f"Server IP: {SERVER_IP}")
except:
    logging.warning("Could not fetch server IP -- continuing anyway")
...
def compute_orb_levels(candles):
    ...
    return {
        ...
        "orb_open": round(orb_open, 2), "orb_range": round(orb_range, 2),
        "prev_close": round(prev_day_close, 2) if prev_day_close else None,
        "gap_size": gap_size, "gap_direction": gap_direction,
...
def compute_signal(candles, orb):
    ...
    details = {"orb_high": orb_high, "orb_low": orb_low, "orb_range": orb_range,
    ...
    adx = safe(adx_s); pdi = safe(pdi_s); mdi = safe(mdi_s)
    rsi_s = calc_rsi(closes, RSI_LEN); rsi = safe(rsi_s)
    atr_s = calc_atr(highs, lows, closes, ATR_LEN); atr = safe(atr_s)
    ema_f = ema(closes, EMA_FAST); ema_s_arr = ema(closes, EMA_SLOW)
    ...
    bull_score = sum([
        int(last_close > breakout_high), int(gap_dir == "GAP_UP"),
        int(trend_bull), int(above_vwap), int(pdi_strong),
        int(rsi is not None and 45 < rsi < 75),
    ])
    bear_score = sum([
        int(last_close < breakout_low), int(gap_dir == "GAP_DOWN"),
        int(trend_bear), int(below_vwap), int(mdi_strong),
...
class RiskManager:
    ...
    def check_can_trade(self, groww):
        checks = [
            self._check_orb_formed(), self._check_time_window(),
            self._check_max_trades(), self._check_daily_loss(),
        ]
        if not PAPER_TRADE:
    ...
    def _check_max_trades(self):
        ...
        if c >= MAX_TRADES_DAY: return False, "MAX TRADES: %d/%d" % (c, MAX_TRADES_DAY)
        return True, "Trades %d/%d" % (c, MAX_TRADES_DAY)

    def _check_daily_loss(self):
        pnl = self.state.get("daily_pnl_rupees", 0)
        if pnl <= -MAX_DAILY_LOSS_RUPEES: return False, "MAX LOSS: Rs.%s" % pnl
    ...
    def _check_open_positions(self, groww):
        try:
            ...
            if open_pos: return False, "POSITION OPEN: %s" % [p['trading_symbol'] for p in open_pos]
            return True, "No open positions"
        except Exception as e:
            return False, "POSITION CHECK FAILED: %s" % e

    def check_premium_range(self, ltp):
        if not ltp or ltp <= 0: return False, "PREMIUM: Could not fetch"
        if ltp < MIN_OPTION_PREMIUM: return False, "PREMIUM TOO LOW: Rs.%s" % ltp
        if ltp > MAX_OPTION_PREMIUM: return False, "PREMIUM TOO HIGH: Rs.%s" % ltp
        return True, "Premium OK: Rs.%s" % ltp
    ...
    def check_capital_exposure(self, ltp, qty):
        ...
        if exposure > max_exp: return False, "EXPOSURE: Rs.%.0f (%.1f%%) > max" % (exposure, pct)
    ...
    def check_spread(self, groww, symbol):
        ...
        try:
            ...
            if spread_pct > MAX_SPREAD_PCT: return False, "SPREAD: %.1f%%" % spread_pct
            return True, "Spread OK: %.1f%%" % spread_pct
        except Exception as e:
            return True, "Spread skipped: %s" % e
...
def get_expiry_date():
    today = ist_now().date(); now = ist_now()
...
def build_symbol(strike, opt_type, expiry=None):
    ...
    yy = expiry.strftime("%y"); m_code = _nse_month_code(expiry.month); dd = expiry.strftime("%d")
...
def login():
    ...
    logging.info("[OK] Login OK (fresh)")
    return groww
...
def _parse_candles(raw):
    ...
    for c in raw:
        ...
        result.append({"ts": ts, "open": float(c[1]), "high": float(c[2]),
...
def _get_nifty_quote(groww):
    try:
        ...
        logging.warning("Nifty quote failed: %s" % e)
    return None

def fetch_candles(groww):
    now = ist_now(); today = now.strftime("%Y-%m-%d")
    ...
    try:
        ...
        if raw:
            ...
            cache["date"] = today; _save_candles(cache)
            logging.info("Candles: %d (inc=%s)" % (len(cache["candles"]), is_incremental))
            return cache["candles"]
        elif is_incremental and len(existing) >= 30:
            logging.info("No new candles -- cache (%d)" % len(existing))
            return existing
    except Exception as e:
        logging.warning("Candle API failed: %s" % e)
    if existing and len(existing) >= 30: return existing
...
def get_option_ltp(groww, symbol):
    try:
        ...
        logging.warning("Option LTP failed for %s: %s" % (symbol, e))
        return None
...
def _discover_expiry_from_csv(groww, strike, opt_type):
    try:
        ...
        today = ist_now().date(); best_expiry, best_row = None, None
        ...
        logging.warning("CSV discovery failed: %s" % e)
    return None, None

def _discover_expiry_from_chain(groww, strike, opt_type, calculated_expiry):
    today = ist_now().date(); calc_str = calculated_expiry.strftime("%Y-%m-%d")
    for offset in range(0, 8):
        ...
        try:
            ...
            for sk in [str(strike), "%s.0" % strike]:
            ...
            if available:
                ...
                for sk in [str(nearest), "%s.0" % nearest]:
                    if sk in strikes_data and opt_type in strikes_data[sk]:
                        opt_data = strikes_data[sk][opt_type]
                        tsym = opt_data.get("trading_symbol", "")
                        ltp = float(opt_data.get("ltp", 0))
...
def get_valid_option_symbol(groww, strike, opt_type):
    expiry = get_expiry_date(); expiry_str = expiry.strftime("%Y-%m-%d")
    logging.info("Resolving: %s %s exp=%s" % (strike, opt_type, expiry_str))
    sym = build_symbol(strike, opt_type, expiry)
    try:
        q = groww.get_quote(exchange="NSE", segment="FNO", trading_symbol=sym)
        ltp = float(q.get("last_price", 0))
        if ltp > 0:
            logging.info("OK (direct): %s Rs.%s" % (sym, ltp))
            return sym, ltp
    except Exception as e:
        logging.warning("Direct %s failed: %s" % (sym, e))
    chain_found = False
    try:
        ...
        logging.error("Chain failed: %s" % e)
    if not chain_found:
    ...
    if csv_sym:
        ...
        return csv_sym, 0
    logging.error("ALL FAILED: %s %s" % (strike, opt_type))
    return None, None
...
def place_entry_order(groww, symbol, qty, txn):
    if PAPER_TRADE:
        ...
        logging.info("[PAPER] ENTRY | %s | Qty:%d" % (symbol, qty))
        return fake_id
    try:
        res = groww.place_order(
            ...
            transaction_type=txn, order_reference_id="SNP%s" % ist_now().strftime("%H%M%S"))
        oid = res.get("groww_order_id", "N/A")
        logging.info("ENTRY | %s | Qty:%d | ID:%s" % (symbol, qty, oid))
        return oid
    except Exception as e:
        logging.error("ENTRY FAILED: %s" % e)
        return None
...
def monitor_and_exit(groww, state):
    ...
    if PAPER_TRADE:
        logging.info("[PAPER] MONITOR | %s (no real LTP)" % symbol)
        return False
    current_ltp = get_option_ltp(groww, symbol)
    if current_ltp is None:
        logging.warning("MONITOR: Cannot fetch LTP for %s" % symbol)
        return False
    pnl_per_unit = current_ltp - entry_premium
    pnl_total = pnl_per_unit * qty
    logging.info("MONITOR | %s | Entry:Rs.%s | Now:Rs.%s | P&L:Rs.%+.0f" % (symbol, entry_premium, current_ltp, pnl_total))
    exit_reason = None
    if pnl_total >= TAKE_PROFIT_RUPEES:
        exit_reason = "[PROFIT] TAKE PROFIT Rs.%+.0f" % pnl_total
    elif pnl_total <= -STOP_LOSS_RUPEES:
        exit_reason = "[LOSS] STOP LOSS Rs.%+.0f" % pnl_total
    if exit_reason is None:
        return False
    logging.info("%s | Selling %s Qty:%d" % (exit_reason, symbol, qty))
    try:
        groww.place_order(
            ...
            order_reference_id="EXIT%s" % ist_now().strftime("%H%M%S"))
        logging.info("EXIT ORDER PLACED | %s" % symbol)
    except Exception as e:
        logging.error("EXIT FAILED: %s" % e)
        return False
    ...
    save_state(state)
    logging.info("CLOSED | %s | Daily P&L:Rs.%+.0f" % (exit_reason, state["daily_pnl_rupees"]))
    return True
...
def cancel_and_squareoff(groww, state):
    ...
    if trades and not trades[-1].get("exited", False):
        ...
        entry_premium = last_trade["entry_premium"]
        if PAPER_TRADE:
            logging.info("[PAPER] EOD EXIT | %s" % symbol)
            ...
            try:
                groww.place_order(
                    ...
                    product="MIS", order_type="MARKET",
                    transaction_type="SELL",
                    order_reference_id="EOD%s" % ist_now().strftime("%H%M%S"))
                logging.info("EOD EXIT | %s Qty:%d" % (symbol, qty))
            except Exception as e:
                logging.error("EOD EXIT FAILED: %s" % e)
        pnl = (current_ltp - entry_premium) * qty
        last_trade["exited"] = True
        last_trade["exit_ltp"] = current_ltp
        last_trade["exit_pnl"] = round(pnl, 0)
        last_trade["exit_reason"] = "EOD SQUAREOFF"
        last_trade["exit_time"] = ist_now().strftime("%Y-%m-%d %H:%M:%S")
        state["daily_pnl_rupees"] = state.get("daily_pnl_rupees", 0) + pnl
        save_state(state)
    logging.info("DAILY SUMMARY | Trades:%d | P&L:Rs.%+.0f" % (len(trades), state.get("daily_pnl_rupees", 0)))
...
def main():
    ...
    if trades and not trades[-1].get("exited", False):
        ...
        if not exited:
            logging.info("[%s] Position open. Monitoring..." % now.strftime("%H:%M"))
        return
    ...
    logging.info("=== %s | %s | Trades:%d/%d | P&L:Rs.%+.0f ===" % (
        mode_tag, now.strftime("%Y-%m-%d %H:%M:%S"), state["trade_count"], MAX_TRADES_DAY,
    ...
    if not candles:
        logging.error("No candles"); return

    orb = load_orb()
    if orb is None:
        if ist_now() < ist_now().replace(hour=9, minute=30, second=0):
            logging.info("ORB forming..."); return
        orb = compute_orb_levels(candles)
        if orb is None:
            logging.error("Cannot compute ORB"); return
        save_orb(orb)
        logging.info("ORB | H:%s L:%s R:%s GAP:%s(%s)" % (
            orb["orb_high"], orb["orb_low"], orb["orb_range"],
            orb["gap_direction"], orb["gap_size"]))

    result = compute_signal(candles, orb)
    signal = result["signal"]; confidence = result.get("confidence")
    d = result.get("details", {})

    logging.info("  Signal:%s | Conf:%s | %s" % (signal, confidence, d.get("trigger", "--")))
    ...
    if symbol is None:
        ...
        if symbol is None:
            logging.error("Cannot resolve symbol"); return
        strike = atm

    logging.info("Resolved: %s | Rs.%s | Nifty:%s" % (symbol, option_ltp, nifty_ltp))
    ...
    for check_fn, args in [
        ...
        if not ok:
            logging.warning("BLOCKED: %s" % msg); return
        logging.info(msg)

    tp_at = round(option_ltp + TAKE_PROFIT_RUPEES / qty, 1)
    sl_at = round(max(option_ltp - STOP_LOSS_RUPEES / qty, 1.0), 1)
    logging.info("PLAN | Entry:~Rs.%s | TP:Rs.%s (+Rs.%d) | SL:Rs.%s (-Rs.%d)" % (
        option_ltp, tp_at, TAKE_PROFIT_RUPEES, sl_at, STOP_LOSS_RUPEES))

    oid = place_entry_order(groww, symbol, qty, groww.TRANSACTION_TYPE_BUY)
    if not oid: return

    time.sleep(1)
    entry_premium = option_ltp if PAPER_TRADE else (get_option_ltp(groww, symbol) or option_ltp)

    tp_price = round(entry_premium + TAKE_PROFIT_RUPEES / qty, 1)
    sl_price = round(max(entry_premium - STOP_LOSS_RUPEES / qty, 1.0), 1)

    state["trade_count"] += 1
    state["trades"].append({
        "signal": signal, "confidence": confidence,
        "symbol": symbol, "qty": qty,
        "entry_id": str(oid), "entry_premium": entry_premium,
        "target": tp_price, "sl": sl_price,
        ...
        "paper": PAPER_TRADE, "exited": False,
        "time": now.strftime("%Y-%m-%d %H:%M:%S")
    })
    save_state(state)

    tag = "[PAPER]" if PAPER_TRADE else "[LIVE]"
    logging.info("%s TRADE | %s %s | %s" % (tag, signal, confidence, symbol))
    logging.info("   Entry:Rs.%s | TP:Rs.%s (+Rs.%d) | SL:Rs.%s (-Rs.%d)" % (
        entry_premium, tp_price, TAKE_PROFIT_RUPEES, sl_price, STOP_LOSS_RUPEES))
    logging.info("   Bot monitors every cron run -- no OCO needed")
