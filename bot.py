...
def build_symbol(strike, opt_type):
    ...
    Confirmed from option chain: trading_symbol = NIFTY2640722950CE
    where 26=year, 4=month(single digit for Apr), 07=day
    """
    expiry = get_expiry_date()
    yy = expiry.strftime("%y")                    # "26"
    m_code = _nse_month_code(expiry.month)        # "4" for April
    dd = expiry.strftime("%d")                    # "07"
...
def fetch_candles(groww):
    ...
    # Confirmed working: trading_symbol="NIFTY", exchange="NSE", segment="CASH"
    try:
...
def get_valid_option_symbol(groww, strike, opt_type):
    """Resolve option symbol using option chain API.
    Primary: build_symbol (NIFTY2640722950CE format)
    Fallback: get_option_chain to discover exact trading_symbol
    """
    # Try 1: Direct build
    ...
    # Try 2: Use option chain API to find exact symbol
    expiry = get_expiry_date()
    expiry_formats = [
    ...
    for exp_fmt in expiry_formats:
        try:
            ...
            # Search through chain for our strike
            options = []
            if isinstance(chain, dict):
                # Try common response structures
                for key in ["options", "data", "option_chain", "ce", "pe",
                            "call_options", "put_options", "strikes"]:
                    if key in chain:
                        val = chain[key]
                ...
                # If none of the known keys, flatten all lists in response
                if not options:
            ...
            for opt in options:
                ...
                opt_strike = opt.get("strike_price", opt.get("strikePrice",
                              opt.get("strike", 0)))
                opt_t = opt.get("option_type", opt.get("optionType",
                         opt.get("type", "")))

                try:
                    opt_strike = float(opt_strike)
                except:
                    continue

                if int(opt_strike) == strike and opt_type.upper() in str(opt_t).upper():
                    if tsym:
                        try:
                            ...
                            logging.debug(f"Chain symbol {tsym} quote failed")
                            # Still return it — might work for order placement
                            opt_ltp = float(opt.get("last_price", opt.get("ltp", 0)))
                            if opt_ltp > 0:
                                logging.info(f"Symbol from chain (no quote): {tsym} | Chain LTP: Rs.{opt_ltp}")
                                return tsym, opt_ltp

            logging.info(f"Option chain OK with expiry={exp_fmt} but strike {strike} {opt_type} not found")

        except Exception as e:
            logging.debug(f"Option chain with expiry={exp_fmt} failed: {e}")
            continue

    logging.error(f"Cannot resolve symbol for strike={strike} type={opt_type}")
    return None, None
...
def main():
    ...
    # ── Resolve symbol via direct build + option chain fallback
    symbol, option_ltp_pre = get_valid_option_symbol(groww, strike, opt_type)
    if symbol is None:
        itm_strike = strike - 50 if opt_type == "CE" else strike + 50
        logging.warning(f"ATM {strike} failed, trying ITM {itm_strike}")
        symbol, option_ltp_pre = get_valid_option_symbol(groww, itm_strike, opt_type)
        if symbol is None:
            logging.error(f"Cannot resolve symbol for {strike}/{itm_strike} {opt_type}"); return
        strike = itm_strike

    logging.info(f"Resolved: {symbol} | LTP: Rs.{option_ltp_pre} | Nifty: {ltp} | Strike: {strike}")
