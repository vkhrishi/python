...
def compute_impulse_leg(candles):
    ...
    Returns the swing high, swing low, direction, and fib levels.
    """
    today = ist_now().date()
    impulse_candles = []
    ...
    # If close of last impulse candle > open of first -> bullish impulse (up move)
    # Retracement = pullback down -> buy at fib support
    first_open = impulse_candles[0]["open"]
    last_close = impulse_candles[-1]["close"]

    # Also check where swing high/low occurred relative to each other
    ...
    if last_close > first_open and high_idx > low_idx:
        direction = "BULL"  # Impulse up, expect pullback down to fib, then buy
    elif last_close < first_open and low_idx > high_idx:
        direction = "BEAR"  # Impulse down, expect pullback up to fib, then sell
    else:
        # Ambiguous - use close vs open as tiebreaker
        if last_close > first_open:
            direction = "BULL"
        elif last_close < first_open:
            direction = "BEAR"
        else:
            return None  # Doji impulse, skip

    # Calculate Fibonacci retracement levels
    if direction == "BULL":
        # Bullish impulse: swing_low = bottom, swing_high = top
        # Retracement levels are measured DOWN from top
        ...
        # Extension targets (above swing high)
        ext_1272 = round(swing_low + (impulse_size * FIB_EXT_1272), 2)
        ext_1618 = round(swing_low + (impulse_size * FIB_EXT_1618), 2)
    else:
        # Bearish impulse: swing_high = top, swing_low = bottom
        # Retracement levels are measured UP from bottom
        ...
        # Extension targets (below swing low)
