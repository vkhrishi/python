...
# =============================================================
# =============================================================
#  SECTION 7 - FETCH CANDLES (FIXED FOR NEW API)
# =============================================================
...
def fetch_candles(groww):
    ...
    # Signature: (exchange, segment, groww_symbol, start_time, end_time, candle_interval)
    # candle_interval uses strings: "1minute", "5minute", "10minute", "15minute",
    ...
    try:
        ...
        if raw:
            method_used = "get_historical_candles"
            logging.info("Candle OK via %s: %d raw" % (method_used, len(raw)))
    except Exception as e:
        logging.warning("get_historical_candles failed: %s" % str(e))

    # METHOD 2: Try "NIFTY 50" as groww_symbol
    if not raw:
        for sym in ["NIFTY 50", "NIFTY50", "Nifty 50", "Nifty"]:
            try:
                res = groww.get_historical_candles(
                    exchange="NSE",
                    segment="CASH",
                ...
                raw = _extract_raw(res)
                if raw:
                    method_used = "get_historical_candles(%s)" % sym
                    logging.info("Candle OK via %s: %d raw" % (method_used, len(raw)))
                    break
            except Exception as e:
                logging.info("get_historical_candles(%s) failed: %s" % (sym, str(e)))

    # METHOD 3: Try SDK interval constant
    if not raw:
        interval_const = getattr(groww, "CANDLE_INTERVAL_MIN_5", None)
        if interval_const:
            try:
                res = groww.get_historical_candles(
                    exchange="NSE",
                ...
                raw = _extract_raw(res)
                if raw:
                    method_used = "get_historical_candles(const=%s)" % str(interval_const)
                    logging.info("Candle OK via %s: %d raw" % (method_used, len(raw)))
            except Exception as e:
                logging.warning("get_historical_candles(const) failed: %s" % str(e))

    # METHOD 4: Old deprecated API (fallback)
    if not raw:
        try:
            ...
            raw = _extract_raw(res)
            if raw:
                method_used = "get_historical_candle_data"
                logging.info("Candle OK via deprecated API: %d raw" % len(raw))
        except Exception as e:
            logging.warning("Deprecated API failed: %s" % str(e))
    ...
    if raw:
        ...
        if new_candles:
            ...
            logging.info("Candles: %d total | %d today | method=%s" % (
                len(cache["candles"]), today_count, method_used))
