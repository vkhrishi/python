...
def _parse_candles(raw):
    """Normalize candle rows from get_historical_candles response.
    c[0] is 'YYYY-MM-DDTHH:MM:SS' (ISO 8601 with T separator).
    c[6] is open interest — ignored for index data (null).
    """
    result = []
    for c in raw:
        if len(c) < 5:
            continue
        # API returns ISO timestamp like "2025-09-24T10:30:00" — normalise to space-separated
        ts = str(c[0]).replace("T", " ") if c[0] is not None else ""
        result.append({
            "ts"    : ts,
            "open"  : float(c[1]),
            "high"  : float(c[2]),
...
def _call_historical(groww, start_dt, end_dt):
    """Call get_historical_candles with the correct Groww symbol for NIFTY index."""
    return groww.get_historical_candles(
        exchange        = groww.EXCHANGE_NSE,
        segment         = groww.SEGMENT_CASH,
        groww_symbol    = "NSE-NIFTY",          # Groww symbol format: EXCHANGE-SYMBOL
        start_time      = start_dt,
        end_time        = end_dt,
...
def fetch_candles(groww):
    ...
    try:
        res = _call_historical(groww, start_dt, end_dt)
        ...
        err = str(e).lower()
        if any(k in err for k in ("forbidden", "unauthorized", "401", "403")):
            logging.warning(f"Historical API auth error ({e}) — refreshing token and retrying")
            try:
                groww = login(force=True)
                res2  = _call_historical(groww, start_dt, end_dt)
                raw2  = res2.get("candles", []) if isinstance(res2, dict) else []
                if raw2:
                    new_candles = _parse_candles(raw2)
                    if is_incremental:
                        existing_ts = {c["ts"] for c in existing if c.get("ts")}
                        merged = existing + [c for c in new_candles if c.get("ts") not in existing_ts]
                        cache["candles"] = merged[-200:]
                        logging.info(f"Retry OK: +{len(new_candles)} candle(s) | total={len(cache['candles'])}")
                    else:
                        cache["candles"] = new_candles[-200:]
                        logging.info(f"Retry OK: full load {len(cache['candles'])} candles — READY")
                    _save_candles(cache)
                    return [{k: v for k, v in c.items() if k != "ts"} for c in cache["candles"]]
            except Exception as e2:
                logging.warning(f"Historical API retry failed: {e2} — falling back to live quote")
        else:
