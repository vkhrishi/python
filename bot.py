...
#    - Multi-trigger entry system (5 independent triggers)
# ============================================================
...
LOT_SIZE         = 65
LOTS_TO_TRADE    = 1
HIGH_ONLY        = False        # Allow MED confidence trades too (4/7+ confluence)

TARGET_MULT      = 1.5          # 1.5× target — realistic for 5-min scalps
SL_MULT          = 0.5          # 50% SL — wider to avoid noise stops
USE_SMART_ORDERS = True

SQUAREOFF_HOUR          = 15
SQUAREOFF_MIN           = 10
NO_NEW_TRADE_HOUR       = 14
NO_NEW_TRADE_MIN        = 45    # Extended from 14:15 to 14:45
OPENING_RANGE_MINUTES   = 15    # Industry standard 15 min
NO_TRADE_BEFORE_SQ_MINS = 20

MAX_TRADES_DAY            = 5   # More room with relaxed filters
MAX_DAILY_LOSS_RUPEES     = 2000
MAX_CONSECUTIVE_LOSSES    = 3   # Was 2 — less aggressive kill switch
MAX_OPEN_POSITIONS        = 1
COOLDOWN_MINUTES          = 10  # Faster re-entry
CAPITAL_RUPEES            = 50000
MAX_CAPITAL_EXPOSURE_PCT  = 24
MAX_SPREAD_PCT            = 3.0
MIN_OPTION_PREMIUM        = 40  # Wider range for near-expiry
MAX_OPTION_PREMIUM        = 250 # Allows slightly ITM picks
...
SFP_LOOKBACK= 20
PIVOT_LEN   = 5
MIN_CONFLUENCE = 3        # Score 3/7 + trigger is still filtered
MIN_ATR_POINTS = 8        # 5-min Nifty ATR is often 10-14; 8 filters only dead sessions
VWAP_SESSION_BARS = 75    # ~1 full trading session of 5-min bars
...
# ═════════════════════════════════════════════════════════════
#  SECTION 3 — SIGNAL ENGINE (Multi-trigger system)
# ═════════════════════════════════════════════════════════════
...
def compute_signals(candles):
    ...
    # ── CVD over today's session only (not all 5 days of candles)
    ...
    # ── Trend: relaxed — only price vs EMA21 direction
    #    Ribbon alignment (EMA21>55>144) is a bonus, not a gate
    trend_bull  = lc > le21
    trend_bear  = lc < le21
    ...
    # ── Session VWAP bias + overextension guard (relaxed to 2.5× ATR)
    ...
    adx_strong = ladx >= 20     # Standard trending threshold (was 25)
    ...
    # ── Ribbon alignment bonus (+1, capped at 7)
    ...
    # Trigger 2: EMA 8/21 spread — sustained momentum (price riding above EMA8 > EMA21)
    ...
    # Trigger 3: SFP near supply/demand zone
    sfp_bull_trigger = bull_sfp and near_demand
    sfp_bear_trigger = bear_sfp and near_supply

    # Trigger 4: RSI reversal from extreme + engulfing candle
    ...
    # Trigger 5: Pullback to EMA21 in trend (bounce entry)
    ...
    # ── VWAP soft filter — adds/subtracts from score instead of blocking
    ...
    # Only block if massively overextended
    ...
    # ── Identify which trigger fired (for logging)
    trigger_name = "none"
    if signal != "NO_TRADE":
        if "CE" in signal:
    ...
    return {
        ...
        "details": {
            ...
            "ema_spread_bull": ema_spread_bull,
            "ema_spread_bear": ema_spread_bear,
...
def get_expiry_str():
    ...
    if days_ahead == 0:
        # Today is Tuesday — use today if before 15:30, else next week
        now = ist_now()
...
def _call_historical(groww, start_dt, end_dt):
...
def _call_historical_legacy(groww, start_dt, end_dt):
...
def fetch_candles(groww):
    ...
    def _try_api(call_fn, label):
        try:
            res = call_fn(groww, start_dt, end_dt)
            raw = res.get("candles", []) if isinstance(res, dict) else []
            if raw:
                new_candles = _parse_candles(raw)
                _merge_candles(existing, new_candles, is_incremental, cache, label)
                _save_candles(cache)
                return [{k: v for k, v in c.items() if k != "ts"} for c in cache["candles"]]
            if is_incremental and len(existing) >= min_needed:
                logging.info(f"{label}: no new candles — reusing cache ({len(existing)})")
                return [{k: v for k, v in c.items() if k != "ts"} for c in existing[-200:]]
            logging.warning(f"{label}: returned empty data")
        except Exception as e:
            logging.warning(f"{label} failed: {e}")
        return None

    result = _try_api(_call_historical, "HistoricalAPI-v2")
    if result is not None:
        return result

    result = _try_api(_call_historical_legacy, "HistoricalAPI-v1(legacy)")
    if result is not None:
        return result

    # Fallback: live quote snapshot
    try:
