...
def compute_signal(candles, orb, now=None):
    ...
    if avg_vol > 0:
        ...
        details["vol_ratio"] = "N/A (no vol data)"  # index has no volume; filter silently skipped
...
def run_backtest_nifty(days=30):
    ...
    _print_bt_report_nifty(trades, signals, days, diag=diag)


def _print_diag_nifty(diag):
    if not diag:
        return
    print("-" * 60)
...
def _print_bt_report_nifty(trades, signals, days, diag=None):
    print("\n" + "=" * 60)
