# ============================================================
#  NIFTY SNIPER v1 — BAG + ORB + FVG Combined Strategy
...
#
#  API BUDGET (Groww limits: 10/sec, 300/min Live Data):
#    • Login: cached (1 call/day vs 360/day)
#    • Candles: 1 call/run
#    • Quote: 1 call only when signal fires
#    • Symbol resolve: 1-2 calls only when trading
#    • Total: ~2 calls/run idle, ~6 calls/run trading
#    • At 5-min cron: 2×72 = 144 calls/day (well under limit)
# ============================================================
...
# ── BAG (Breakaway Gap) ──
MIN_GAP_POINTS       = 30   # Minimum gap to qualify
...
# ── Market regime ──
MIN_ADX_FOR_TRADE    = 20   # Stricter than before
ADX_LEN              = 14
ATR_LEN              = 14
RSI_LEN              = 14
EMA_FAST             = 9
...
def compute_signal(candles, orb):
    ...
    # ── STEP 1: Determine bias from BAG + ORB ──
    ...
    # Score bullish/bearish confluence (need 3+ to trade)
    bull_score = sum([
        ...
        int(rsi is not None and 45 < rsi < 75),     # RSI healthy (not overbought)
    ])
    ...
    bear_score = sum([
        ...
        int(rsi is not None and 25 < rsi < 55),     # RSI healthy (not oversold)
    ])

    details["bull_score"] = f"{bull_score}/6"
    details["bear_score"] = f"{bear_score}/6"

    # Need ORB breakout + at least 2 more confirmations (total 3+)
    ...
    if fvg_top is not None:
        ...
        # For FVG entry: SL is just beyond the FVG zone
        if direction == "BULL":
        ...
        # No FVG found: use ORB level as SL reference
        if direction == "BULL":
    ...
    confidence = "HIGH" if score >= 4 else "MED"
...
# ═════════════════════════════════════════════════════════════
#  SECTION 6 — LOGIN (CACHED — saves ~350 API calls/day)
# ═════════════════════════════════════════════════════════════
...
def fetch_candles(groww):
    """Single API call per run — respects rate limits."""
    now = ist_now()
    ...
    if existing and existing[-1].get("ts"):
        try:
            ...
            start_dt = (now - datetime.timedelta(days=2)).strftime("%Y-%m-%d 09:15:00")
    else:
        start_dt = (now - datetime.timedelta(days=2)).strftime("%Y-%m-%d 09:15:00")

    # Single candle API call
    try:
        ...
        if raw:
            ...
            _save_candles(cache)
            logging.info(f"Candles: {len(cache['candles'])} total")
...
def place_oco_order(groww, symbol, qty, entry_premium, sl_price, target_price):
    ...
    logging.info(f"OCO | E:Rs.{entry_premium} T:Rs.{target_price} SL:Rs.{sl_price}")
    try:
        res = groww.create_smart_order(
            smart_order_type=groww.SMART_ORDER_TYPE_OCO,
            reference_id=f"OCO{ist_now().strftime('%H%M%S')}",
            ...
            target={"trigger_price": fmt(round(target_price * 0.99, 1)),
                    "order_type": groww.ORDER_TYPE_LIMIT, "price": fmt(target_price)},
        ...
        logging.info(f"OCO OK | ID:{sid}")
        return sid
    except Exception as e:
        logging.error(f"OCO FAILED: {e}")
        logging.warning(f"⚠️ MANUAL: SL=Rs.{sl_price} T=Rs.{target_price}")
        return None
...
def main():
    ...
    logging.info(f"═══ {now.strftime('%Y-%m-%d %H:%M:%S')} | Trades:{state['trade_count']}/{MAX_TRADES_DAY} | P&L:Rs.{state.get('daily_pnl_rupees',0):+.0f} ═══")
    ...
    # ── SIGNAL (zero API calls — pure computation) ──
    ...
    if confidence != "HIGH":
        logging.info(f"  → {confidence} confidence — skip (HIGH only)")
        return

    # ── RISK CHECK (1 API call for positions) ──
    ...
    logging.info(f"✅ TRADE | {signal} {confidence} | {symbol}")
