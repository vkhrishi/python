...
# -- Local data/log directory (cross-platform; created next to this script) --
BASE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "crypto-scalper")
...
FIB_FILE    = os.path.join(DATA_DIR, "fib_levels.json")
CANDLE_FILE = os.path.join(DATA_DIR, "candles.json")
MONITOR_PID = os.path.join(DATA_DIR, "monitor.pid")
...
TRAILING_SL_STEP     = 0.60   # Lock 60% of profit
...
def check_fib_signal(candles, fib_data):
    ...
    if direction == "BULL":
        ...
        if last_close > last_open and candle_range > 0:
            if candle_body / candle_range >= BOUNCE_MIN_BODY_PCT:
                bounce_confirmed = True
                details["bounce_type"] = "BULLISH"
...
def place_sl_tp_orders(client, direction, qty, sl_price, tp_price):
    """Place stop-loss and take-profit as stop-market orders."""
    ...
    try:
        ...
        logging.error("SL/TP order failed: %s" % str(e))
        return False
...
def run_monitor(client, state):
    ...
    def on_message(_, msg):
        ...
        minutes_held = (datetime.datetime.utcnow() - entry_time).total_seconds() / 60
...
def run_backtest(days=5):
    ...
    _print_backtest_report(trades, signals, days)


def _print_backtest_report(trades, signals, days):
    print("\n" + "=" * 56)
...
def main():
    ...
    place_sl_tp_orders(client, direction, pos["qty"], pos["sl_price"], pos["tp_price"])
