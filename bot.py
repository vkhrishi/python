...
# -- Exit: Fixed Rs profit/loss (NO OCO) --
...
def compute_orb_levels(candles):
    ...
    return {
        ...
        "orb_open": round(orb_open, 2), "orb_close": round(orb_close, 2),
...
def place_entry_order(groww, symbol, qty, txn):
    if PAPER_TRADE:
        ...
        logging.info(f"?? PAPER ENTRY | {symbol} | Qty:{qty}")
        return fake_id
    try:
...
def monitor_and_exit(groww, state):
    """Check open position. Exit at fixed Rs profit/loss."""
    ...
    if PAPER_TRADE:
        ...
        logging.info(f"?? PAPER MONITOR | {symbol} (no real LTP in paper mode)")
        return False
    ...
    try:
        groww.place_order(
            ...
            order_reference_id=f"EXIT{ist_now().strftime('%H%M%S')}")
...
def cancel_and_squareoff(groww, state):
    ...
    if trades and not trades[-1].get("exited", False):
        ...
        if PAPER_TRADE:
            logging.info(f"?? PAPER EOD EXIT | {symbol}")
            current_ltp = entry_premium
        else:
            ...
            try:
                groww.place_order(
                    ...
                    product="MIS", order_type="MARKET",
                    transaction_type="SELL",
                    order_reference_id=f"EOD{ist_now().strftime('%H%M%S')}")
                logging.info(f"EOD EXIT | {symbol} Qty:{qty}")
            except Exception as e:
                logging.error(f"EOD EXIT FAILED: {e}")
...
def main():
    ...
    if not ok:
        logging.warning(f"BLOCKED: {reason}"); return
    ...
    for check_fn, args in [
        ...
        if not ok:
            logging.warning(f"BLOCKED: {msg}"); return
        logging.info(msg)

    tp_at = round(option_ltp + TAKE_PROFIT_RUPEES / qty, 1)
    sl_at = round(max(option_ltp - STOP_LOSS_RUPEES / qty, 1.0), 1)
    logging.info(f"PLAN | Entry:~Rs.{option_ltp} | TP:Rs.{tp_at} (+Rs.{TAKE_PROFIT_RUPEES}) | SL:Rs.{sl_at} (-Rs.{STOP_LOSS_RUPEES})")
