...
#    3. VWAP - Institutional bias filter (above=long, below=short)
#
#  WHY THIS WORKS:
#    - S/R levels are where real orders sit (institutions)
#    - ORB breakout WITH S/R confluence = high probability
#    - VWAP confirms institutional direction
#    - Fewer filters = more trades = edge plays out
#
#  RULES:
#    - 1 trade/day MAX
#    - No trades after 13:00 (more time for moves)
#    - RR minimum 1:1.5 (realistic for options)
#    - Tight SL behind ORB level (not percentage-based)
#    - ATM/slightly ITM options for better delta
#    - 3-min hold period (short, just survive spikes)
# ============================================================
...
LOT_SIZE         = 65
LOTS_TO_TRADE    = 1
ITM_OFFSET       = 50       # Slightly ITM (was 100 - too deep = low gamma)

# -- PAPER TRADE MODE --
PAPER_TRADE      = False

# -- ORB (Opening Range Breakout) --
ORB_MINUTES          = 30       # 9:15 to 9:45 (30 min ORB - more stable)
ORB_BUFFER_PCT       = 0.15     # 0.15% buffer above/below ORB (scales with Nifty)

# -- Support/Resistance --
SR_ZONE_POINTS       = 15       # S/R zone width (+/- 15 pts)
SR_CONFLUENCE_BONUS  = True     # Boost confidence when ORB aligns with S/R

# -- Risk --
# SL is STRUCTURE-BASED (behind ORB level), not percentage
SL_BUFFER_BEHIND_ORB = 10      # SL placed 10 pts behind ORB high/low
MAX_SL_POINTS        = 30      # Max SL in Nifty points (caps risk)
MIN_SL_POINTS        = 8       # Min SL (avoid too tight)
RR_RATIO             = 1.5     # Target = SL x 1.5 (realistic for options)
MIN_RR_RATIO         = 1.5     # Reject if below

# -- Capital risk --
CAPITAL_RUPEES        = 50000
MAX_RISK_RUPEES       = 3000   # Max risk per trade in rupees
MAX_DAILY_LOSS_RUPEES = 3000

# -- Hold time --
MIN_HOLD_MINUTES      = 3      # Reduced: 3 min (less time stuck in losers)
CATASTROPHIC_MAX_LOSS = 2500   # Hard stop during hold

# -- Other limits --
MAX_TRADES_DAY            = 1
MIN_OPTION_PREMIUM        = 80    # Higher min = better delta
MAX_OPTION_PREMIUM        = 350
MAX_SPREAD_PCT            = 2.0
MAX_CAPITAL_EXPOSURE_PCT  = 100

# -- Time --
NO_TRADE_AFTER_HOUR  = 13      # Extended to 1 PM (was 11:30 - too restrictive)
...
TRAILING_SL_TRIGGER  = 0.50    # Start trailing after 50% of target reached
TRAILING_SL_STEP     = 0.40    # Lock in 40% of profit as new SL

# -- Expiry --
NIFTY_EXPIRY_WEEKDAY = 3       # Thursday (check your exchange)
...
# =============================================================
#  SECTION 2 - INDICATORS (MINIMAL - only what we need)
# =============================================================
...
def compute_sr_levels(candles):
    """
    Extract previous day's High, Low, Close as S/R levels.
    Also compute previous day's VWAP close as an extra level.
    These are where institutional orders cluster.
    """
    today = ist_now().date()
    prev_day_candles = []
    prev_date = None
    ...
    pdc = prev_day_candles[-1]["close"]   # Previous Day Close
    pdh = max(c["high"] for c in prev_day_candles)   # Previous Day High
    pdl = min(c["low"] for c in prev_day_candles)    # Previous Day Low
...
def get_nearest_support(price, sr):
    """Find the nearest support level below current price."""
    supports = sorted([
        sr["pdl"], sr["s1"], sr["s2"], sr["pdc"],
        sr.get("pivot", 0)
    ])
    below = [s for s in supports if s < price and s > 0]
    return below[-1] if below else None

def get_nearest_resistance(price, sr):
    """Find the nearest resistance level above current price."""
    resistances = sorted([
        sr["pdh"], sr["r1"], sr["r2"], sr["pdc"],
        sr.get("pivot", 0)
    ])
    above = [r for r in resistances if r > price and r > 0]
    return above[0] if above else None

def price_near_level(price, level, zone=SR_ZONE_POINTS):
    """Check if price is within zone of a S/R level."""
    return abs(price - level) <= zone

def check_sr_confluence(price, direction, sr):
    """
    Check if entry has S/R confluence.
    BULL: price breaking above a resistance = good
    BEAR: price breaking below a support = good
    
    Returns: (has_confluence, level_name, level_value)
    """
    levels = {
        "PDH": sr["pdh"],
        "PDL": sr["pdl"],
    ...
    if direction == "BULL":
        # Price should be breaking above a resistance
        for name, level in levels.items():
            if level > 0 and price_near_level(price, level, SR_ZONE_POINTS * 2):
                if price >= level:
                    return True, name, level
        # Also good: price bouncing off support
        for name, level in levels.items():
            if level > 0 and price > level and (price - level) <= SR_ZONE_POINTS * 3:
                return True, name + "_bounce", level
    else:
        # Price should be breaking below a support
        for name, level in levels.items():
            if level > 0 and price_near_level(price, level, SR_ZONE_POINTS * 2):
                if price <= level:
                    return True, name, level
        # Also good: price rejecting from resistance
        for name, level in levels.items():
...
def compute_orb_levels(candles):
    ...
    for c in candles:
        ...
        if dt.date() != today:
            continue
        t = dt.time()
        # 30-min ORB: 9:15 to 9:45
        if datetime.time(9, 15) <= t < datetime.time(9, 45):
            orb_candles.append(c)

    if len(orb_candles) < 3:  # Need at least 3 five-minute candles
        return None
...
# =============================================================
#  SECTION 5 - SIGNAL ENGINE (SIMPLE: ORB + S/R + VWAP)
# =============================================================

def compute_signal(candles, orb, sr):
    """
    Simple signal logic:
      1. Wait for ORB breakout (price closes outside 30-min range)
      2. Check VWAP alignment (above VWAP = long, below = short)
      3. Check S/R confluence (breakout near a key level = higher confidence)
      4. RSI sanity check (not overbought/oversold)
    
    That's it. No ADX, no FVG, no gap analysis, no body ratio filters.
    """
    ...
    # == ORB BUFFER (percentage-based, scales with Nifty) ==
    buffer = last_close * ORB_BUFFER_PCT / 100  # ~37 pts at 25000
    breakout_high = orb_high + buffer
    breakout_low = orb_low - buffer

    # == DIRECTION: SIMPLE BREAKOUT ==
    direction = None
    ...
    if direction is None:
        details["reason"] = "Inside ORB range (%.1f to %.1f)" % (breakout_low, breakout_high)
        return {"signal": "NO_TRADE", "details": details}

    details["direction"] = direction

    # == FILTER 1: VWAP ALIGNMENT (the key filter) ==
    if vwap is not None:
    ...
    if ef is not None and es is not None:
        ...
        details["ema_aligned"] = True  # No data = assume ok
    ...
    if sr:
        sr_confluence, sr_level_name, sr_level_value = check_sr_confluence(
            last_close, direction, sr)
        details["sr_confluence"] = sr_confluence
        details["sr_level"] = "%s=%.0f" % (sr_level_name, sr_level_value) if sr_level_name else "None"

        # Add S/R levels to details for logging
        details["pdh"] = sr["pdh"]
        details["pdl"] = sr["pdl"]
        details["pdc"] = sr["pdc"]
        details["pivot"] = sr["pivot"]

        # Nearest S/R for SL placement
        if direction == "BULL":
            nearest_support = get_nearest_support(last_close, sr)
            details["nearest_support"] = nearest_support
        else:
            nearest_resistance = get_nearest_resistance(last_close, sr)
            details["nearest_resistance"] = nearest_resistance

    # == CONFIDENCE SCORING (simple) ==
    score = 0
    score += 1  # ORB breakout (always true at this point)
    if vwap is not None and ((direction == "BULL" and last_close > vwap) or
                              (direction == "BEAR" and last_close < vwap)):
        score += 1
    if details.get("ema_aligned", False):
        score += 1
    if sr_confluence:
        score += 1
    if rsi is not None and 30 < rsi < 70:
        score += 1  # RSI in healthy zone

    details["score"] = "%d/5" % score

    # Minimum score: 3/5 (ORB + VWAP + one more)
    if score < 3:
...
class RiskManager:
    ...
    def _check_orb_formed(self):
        now = ist_now()
        orb_end = now.replace(hour=9, minute=45, second=0)  # 30-min ORB
        if now < orb_end:
            return False, "ORB forming: %d min left" % int((orb_end - now).total_seconds() / 60)
    ...
    def check_premium_range(self, ltp):
        ...
        if ltp < MIN_OPTION_PREMIUM:
            return False, "PREMIUM TOO LOW: Rs.%s (min %d)" % (str(ltp), MIN_OPTION_PREMIUM)
        if ltp > MAX_OPTION_PREMIUM:
            return False, "PREMIUM TOO HIGH: Rs.%s (max %d)" % (str(ltp), MAX_OPTION_PREMIUM)
    ...
    def check_spread(self, groww, symbol):
        ...
        try:
            ...
            if spread_pct > MAX_SPREAD_PCT:
                return False, "SPREAD: %.1f%% > %.1f%%" % (spread_pct, MAX_SPREAD_PCT)
...
def calc_sl_tp_structural(entry_premium, nifty_ltp, direction, orb, sr, qty):
    """
    STRUCTURAL SL: Place SL behind the ORB level, not a random percentage.
    
    BULL entry: SL = ORB high - buffer (we entered on breakout above ORB high)
    BEAR entry: SL = ORB low + buffer (we entered on breakout below ORB low)
    
    Then convert Nifty points to approximate option premium movement.
    """
    orb_high = orb["orb_high"]
    orb_low = orb["orb_low"]

    if direction == "BULL":
        # SL in Nifty points: behind ORB high
        sl_nifty = orb_high - SL_BUFFER_BEHIND_ORB
        sl_distance_nifty = nifty_ltp - sl_nifty
    else:
        # SL in Nifty points: behind ORB low
    ...
    # Approximate delta for slightly ITM option: ~0.55-0.65
    # Use 0.55 as conservative estimate
    delta = 0.55
    sl_drop_premium = sl_distance_nifty * delta
    tp_rise_premium = sl_drop_premium * RR_RATIO

    # Cap risk at max allowed
    if sl_drop_premium * qty > MAX_RISK_RUPEES:
        sl_drop_premium = MAX_RISK_RUPEES / qty
        tp_rise_premium = sl_drop_premium * RR_RATIO

    sl_price = round(max(entry_premium - sl_drop_premium, 1.0), 1)
    target_price = round(entry_premium + tp_rise_premium, 1)
    sl_pct = sl_drop_premium / entry_premium * 100 if entry_premium > 0 else 0
    rr = tp_rise_premium / sl_drop_premium if sl_drop_premium > 0 else 0

    return sl_price, target_price, sl_drop_premium, tp_rise_premium, sl_pct, rr
...
def fetch_candles(groww):
    ...
    if len(existing) >= 20 and existing[-1].get("ts"):
        try:
            last_dt = datetime.datetime.strptime(existing[-1]["ts"], "%Y-%m-%d %H:%M:%S")
            if last_dt.date() == now.date():
                start_dt = (last_dt + datetime.timedelta(seconds=1)).strftime("%Y-%m-%dT%H:%M:%S")
                is_incremental = True
            else:
                start_dt = (now - datetime.timedelta(days=7)).strftime("%Y-%m-%dT09:15:00")
        except:
            start_dt = (now - datetime.timedelta(days=7)).strftime("%Y-%m-%dT09:15:00")
    else:
        start_dt = (now - datetime.timedelta(days=7)).strftime("%Y-%m-%dT09:15:00")
    ...
    # Method 1: New API (get_historical_candles)
    combos = [
        {"trading_symbol": "NIFTY",    "exchange": "NSE", "segment": "CASH"},
        {"trading_symbol": "NIFTY 50", "exchange": "NSE", "segment": "CASH"},
        {"trading_symbol": "NIFTY",    "exchange": "NSE", "segment": "INDICES"},
    ]

    for combo in combos:
        # Try new method first
        for method_name in ["get_historical_candles", "get_historical_candle_data"]:
            try:
                method = getattr(groww, method_name, None)
                if method is None:
                    continue

                logging.info("Trying: %s(%s/%s/%s)" % (
                    method_name, combo["trading_symbol"], combo["exchange"], combo["segment"]))

                res = method(
                    trading_symbol=combo["trading_symbol"],
                ...
                if isinstance(res, dict):
                    raw = res.get("candles", res.get("data", []))
                elif isinstance(res, list):
                    raw = res

                if not raw:
                    logging.warning("Empty response from %s: %s" % (method_name, str(res)[:300]))
                    continue

                new_candles = _parse_candles(raw)
                if not new_candles:
                    logging.warning("Parsed 0 candles from %d raw" % len(raw))
                    logging.warning("Sample: %s" % str(raw[0])[:200])
                    continue
                ...
                cache["date"] = today
                _save_candles(cache)
                logging.info("Candles OK: %d | %s | %s/%s" % (
                    len(cache["candles"]), method_name,
                    combo["trading_symbol"], combo["segment"]))
                return cache["candles"]

            except Exception as e:
                logging.warning("%s failed [%s/%s]: %s" % (
                    method_name, combo["trading_symbol"], combo["segment"], str(e)))
                continue

    # Method 2: Build from live quotes (1 candle at a time fallback)
    logging.warning("All historical methods failed. Trying quote-based candle build...")
    try:
        ...
        if q:
            ...
            opn = float(q.get("open_price", ltp))
            vol = float(q.get("volume", 0))
            ts = now.strftime("%Y-%m-%d %H:%M:%S")
            live_candle = {
                "ts": ts, "open": opn, "high": high,
                "low": low, "close": ltp, "volume": vol
            }
            if existing:
                last_ts = existing[-1].get("ts", "")
                if last_ts != ts:
                    existing.append(live_candle)
                else:
                    existing[-1] = live_candle
            else:
                existing = [live_candle]
            cache["candles"] = existing[-300:]
            cache["date"] = today
            _save_candles(cache)
            logging.info("Quote candle added: LTP=%s | Total=%d" % (str(ltp), len(cache["candles"])))
            return cache["candles"]
    except Exception as e:
        logging.warning("Quote fallback failed: %s" % str(e))

    # Method 3: Use cached
    if existing and len(existing) >= 20:
        logging.warning("Using cached candles: %d" % len(existing))
        return existing

    # DEBUG: Discover available methods
    logging.error("ALL CANDLE METHODS FAILED")
    logging.info("=== DEBUG: Available GrowwAPI methods ===")
    api_methods = [m for m in dir(groww) if not m.startswith("_") and callable(getattr(groww, m, None))]
    candle_methods = [m for m in api_methods if any(k in m.lower() for k in ["candle", "histor", "ohlc", "chart"])]
    logging.info("Candle-related methods: %s" % str(candle_methods))
    logging.info("All public methods: %s" % str(api_methods))
...
def main():
    ...
    if sr is None:
        ...
        if sr:
            ...
            logging.warning("No S/R levels (no previous day data)")
    ...
    sl_price, target_price, sl_drop, tp_rise, sl_pct, rr = calc_sl_tp_structural(
        option_ltp, nifty_ltp, direction, orb, sr, qty)
    ...
    sl_price, target_price, sl_drop, tp_rise, sl_pct, rr = calc_sl_tp_structural(
        entry_premium, nifty_ltp, direction, orb, sr, qty)
