...
def _parse_candles(raw):
    """Normalize candles from both API variants:
    - New backtesting API  : c[0] is ISO string "2025-09-24T10:30:00"
    - Legacy historical API: c[0] is epoch seconds integer
    Both are normalised to "YYYY-MM-DD HH:MM:SS" IST strings in the cache.
    """
    result = []
    for c in raw:
        if len(c) < 5:
        ...
        if isinstance(t, (int, float)):
            # Legacy API returns epoch seconds in UTC — convert to IST
...
def _call_historical(groww, start_dt, end_dt):
    """New backtesting API — requires groww_symbol format; higher plan only."""
...
def _call_historical_legacy(groww, start_dt, end_dt):
    """Deprecated historical API — uses trading_symbol + interval_in_minutes.
    Available on basic API plan; returns epoch-second timestamps.
    """
...
def fetch_candles(groww):
    ...
    if len(existing) >= min_needed and existing[-1].get("ts"):
        ...
        # Go back 5 trading days so we always load 150+ candles on cold start
        # (1 day ≈ 75 × 5-min bars; 5 days ≈ 375 bars, we keep last 200)
    ...
    # Attempt 1 — new backtesting API (higher plan)
    result = _try_api(_call_historical, "HistoricalAPI-v2")
    if result is not None:
        return result

    # Attempt 2 — legacy historical API (basic plan)
