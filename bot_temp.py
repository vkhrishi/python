...
def load_candle_cache():
    try:
        ...
        pass
    return {"date":"", "source":None, "candles":[]}
...
def normalize_historical_candles(raw):
    candles = []
    for c in raw:
        if len(c) < 5:
            continue
        ts = str(c[0]) if c[0] is not None else None
        candles.append({
            "ts": ts,
            "open": float(c[1]),
            "high": float(c[2]),
            "low": float(c[3]),
            "close": float(c[4]),
            "volume": float(c[5]) if len(c) > 5 and c[5] not in (None, "") else 0.0
        })
    return candles
...
def fetch_candles(groww):
    ...
    if cache.get("date") != today:
        cache = {"date": today, "source": None, "candles": []}

    existing = cache.get("candles", [])

    # Decide whether to do a full load or an incremental update
    ...
    if len(existing) >= min_needed and existing[-1].get("ts"):
        ...
        # Not enough candles yet — load full history
        start_dt = (now - datetime.timedelta(hours=9)).strftime("%Y-%m-%d %H:%M:%S")

    # Primary path: seed/update from historical candles
    try:
        ...
        new_candles = normalize_historical_candles(raw)
        if new_candles:
            if is_incremental:
                existing_ts = {c["ts"] for c in existing if c.get("ts")}
                merged = existing + [c for c in new_candles if c.get("ts") not in existing_ts]
                cache["candles"] = merged[-150:]
                logging.info(f"Incremental update: +{len(new_candles)} new candle(s) | total={len(cache['candles'])}")
            else:
                cache["candles"] = new_candles[-150:]
                logging.info(f"Full historical load: {len(cache['candles'])} candles | ready for signals")
            cache["source"] = "historical"
            save_candle_cache(cache)
            return [
                {k: v for k, v in candle.items() if k in ("open", "high", "low", "close", "volume")}
                for candle in cache["candles"]
            ]
        # No new candles returned — if incremental and cache is already sufficient, reuse it
        if is_incremental and len(existing) >= min_needed:
            logging.info(f"No new candles from API — reusing cache: {len(existing)} candles")
            return [
                {k: v for k, v in candle.items() if k in ("open", "high", "low", "close", "volume")}
                for candle in existing[-150:]
            ]
        logging.warning("Historical candles returned empty data — using cache/live fallback")
    except Exception as e:
        logging.warning(f"Historical candles failed: {e} — using cache/live fallback")

    # Secondary path: reuse existing cache if it is already sufficient
    if len(existing) >= min_needed:
        logging.info(f"Using cached candles: {len(existing)} | source={cache.get('source') or 'unknown'}")
        return [
            {k: v for k, v in candle.items() if k in ("open", "high", "low", "close", "volume")}
            for candle in existing[-150:]
        ]

    # Final fallback: append a live snapshot to today's cache
    try:
        q = groww.get_quote(
    ...
    cache["candles"] = candles[-150:]
    cache["source"] = cache.get("source") or "live_cache"
    save_candle_cache(cache)
    count = len(cache["candles"])
    logging.info(f"Fallback cache: {count} candles ({'READY' if count >= min_needed else f'need {min_needed-count} more'})")
    return [
        {k: v for k, v in candle.items() if k in ("open", "high", "low", "close", "volume")}
        for candle in cache["candles"]
    ]
