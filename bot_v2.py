...
LOT_SIZE         = 65
LOTS_TO_TRADE    = 1
HIGH_ONLY        = False        # True = only HIGH confluence signals
...
def compute_signals(candles):
    ...
    # ── Trend state (EMA ribbon alignment) ───────────────────
    ...
    delta_list = [v if c > o else (-v if c < o else 0)
                  for c,o,v in zip(closes,opens,volumes)]
    cvd = sum(delta_list)

    # ── ATR state ────────────────────────────────────────────
    atr_avg = sum(v for v in atr_s[-20:] if v is not None) / max(sum(1 for v in atr_s[-20:] if v is not None), 1)
    ...
    # ── Pivot zone proximity ─────────────────────────────────
    # Check if price is near a recent pivot high (supply) or low (demand)
    pivot_highs = []
    pivot_lows  = []
    p = PIVOT_LEN
    ...
    # ── Entry triggers ───────────────────────────────────────
    # Buy trigger: EMA8 crosses above EMA21 OR bull SFP near demand zone
    ...
    buy_trigger  = (ema_cross_bull or (bull_sfp and near_demand)) and trend_state >= 0
    sell_trigger = (ema_cross_bear or (bear_sfp and near_supply)) and trend_state <= 0

    # ── Final signal ─────────────────────────────────────────
    long_high  = long_score >= 5
    long_med   = long_score >= MIN_CONFLUENCE and not long_high
    short_high = short_score >= 5
    short_med  = short_score >= MIN_CONFLUENCE and not short_high

    if buy_trigger and (long_high or long_med):
        signal     = "CE_BUY"
        confidence = "HIGH" if long_high else "MED"
        score      = long_score
    elif sell_trigger and (short_high or short_med):
        signal     = "PE_BUY"
        confidence = "HIGH" if short_high else "MED"
        score      = short_score
    else:
...
#  FIX: uses historical API → instant 200 candles on every run
#  NO more waiting for 50-run warmup!
# ═════════════════════════════════════════════════════════════

def fetch_candles(groww):
    now      = ist_now()
    end_dt   = now.strftime("%Y-%m-%d %H:%M:%S")
    # Fetch last 2 days = ~200 x 5-min candles — plenty for all indicators
    ...
    try:
        ...
        raw = res.get("candles", [])
        if raw:
            candles = [{"open"  : float(c[1]),
                        "high"  : float(c[2]),
                        "low"   : float(c[3]),
                        "close" : float(c[4]),
                        "volume": float(c[5]) if len(c) > 5 and c[5] else 0.0}
                       for c in raw]
            logging.info(f"Historical candles: {len(candles)} fetched — READY instantly")
            return candles
    except Exception as e:
        logging.warning(f"Historical candles failed: {e} — using live quote cache")

    # Fallback: live quote cache (builds up over runs)
    ...
    cache["candles"] = cache["candles"][-200:]
    with open(CANDLE_FILE, "w") as f: json.dump(cache, f)
    count = len(cache["candles"])
    logging.info(f"Live cache: {count} candles ({'READY' if count>=150 else f'need {150-count} more'})")
...
def main():
    ...
    logging.info(f"  Close:{d.get('close','N/A')} | Trend:{d.get('trend','N/A')} | ATR:{d.get('atr_state','N/A')}")
    logging.info(f"  RSI:{d.get('rsi','N/A')} | MACD:{d.get('macd_hist','N/A')} | ADX:{d.get('adx','N/A')} | RVOL:{d.get('rvol','N/A')}")
    logging.info(f"  StochK:{d.get('stoch_k','N/A')} | BullSFP:{d.get('bull_sfp','N/A')} | BearSFP:{d.get('bear_sfp','N/A')}")
